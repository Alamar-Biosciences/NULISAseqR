...
params:
  ...
  # dataDir: '../../../../../tests/testthat/fixtures'
  dataDir: !r NULL
  xmlFiles: !r NULL
  ...
  # reportType: "webApp"
  reportType: "internal"
  allowMissingCurveQuant: !r FALSE
  advancedQC: !r FALSE
  # advancedQC: !r TRUE
  ...
  # Defaults to 4
  minBlankNoMAD: !r 4
  # Un-adjusted p-value cutoff for identifying targets that have overall plate effects in the ANOVA -- pairwise tests are only done for these targets
  ...
  heatMapRel: !r TRUE
  # set output folder where outputFiles below will be written
  outfolder: "."
...
render_table <- function(df, table_name, table_id, highlight = NULL, first_colName="Target", rownames=TRUE, validPageSizes=c(10,25,50,100), defaultPageSize=10, searchLimit=10, HTML=FALSE, custom_columns=NULL, tooltips=NULL) {
  ...
  # Coerce numeric-looking columns to numeric so reactable sorts them numerically
  # rather than as character. Runs for data.frame inputs too, not only matrices.
  # Columns that mix numeric values with text labels (e.g. reverse-curve
  # "High Abundance") are made numeric for sorting, with the labels preserved for
  # display via a hidden companion column + cell renderer.
  label_cols <- character(0)
  for (colname in names(df2)) {
    # already-numeric columns sort numerically and keep their formatting; leave them
    if (is.numeric(df2[[colname]])) next
    orig <- as.character(df2[[colname]])
    suppressWarnings(num_col <- as.numeric(orig))
    # non-numeric, non-blank entries that failed to coerce (i.e. text labels)
    failed <- is.na(num_col) & !is.na(orig) & trimws(orig) != ""
    if (!any(failed)) {
      # entirely numeric (ignoring pre-existing NA / blanks), unless coercion would
      # drop a leading zero (e.g. a zero-padded sample ID "007" becoming 7)
      nonblank <- orig[!is.na(orig) & trimws(orig) != ""]
      zero_padded <- grepl("^0[0-9]", trimws(nonblank))
      if (!any(zero_padded)) df2[[colname]] <- num_col
    } else if (any(!is.na(num_col))) {
      # mixed numeric values and text labels: sort numerically, keep labels
      df2[[paste0(colname, "__label")]] <- ifelse(failed, orig, "")
      df2[[colname]] <- num_col
      label_cols <- c(label_cols, colname)
    }
    # else: entirely non-numeric column (e.g. names, "x (y%)") -> leave as character
  }
  if(rownames){
  ...
  # For columns that mix numeric values with text labels: hide the
  # companion label column and render the parent column so numeric values display
  # (and sort) numerically while text labels (e.g. "High Abundance") still show.
  # Set before the custom_columns merge below, so an explicit custom_columns entry
  # for the same column still takes precedence (it would drop label rendering).
  if (length(label_cols) > 0) {
    for (lc in label_cols) {
      label_source <- paste0(lc, "__label")
      column_defs[[label_source]] <- colDef(show = FALSE)
      column_defs[[lc]] <- colDef(
        sortNALast = TRUE,  # keep labeled (non-numeric) rows at the bottom when sorting
        html = FALSE,       # labels/numbers are plain text; render as text so markup in a label can't inject HTML
        cell = JS(sprintf("
          function(cellInfo) {
            var lbl = cellInfo.row['%s'];
            if (lbl !== null && lbl !== undefined && lbl !== '') { return lbl; }
  ...
  # Add tooltip support with click-to-expand using native HTML <details>/<summary>.
  # This avoids React.useState hooks which crash reactable on pagination/page-size change.
  if(!is.null(tooltips)){
      ...
      column_defs[[tooltip_col]] <- colDef(
        cell = JS(sprintf("
          function(cellInfo) {
            ...
            var html = '<details>' +
              '<summary style=\"color:#0066cc;text-decoration:underline;cursor:pointer\"' +
              ' title=\"Click to show/hide targets\">' + esc(cellInfo.value) + '</summary>' +
              '<div style=\"margin-top:5px;padding:8px;background-color:#f5f5f5;' +
              'border:1px solid #ddd;border-radius:3px;font-size:0.9em;' +
              'max-height:200px;overflow-y:auto;white-space:pre-wrap;' +
              'user-select:text;cursor:text\">' + esc(detailText) + '</div></details>';
  ...
  # Count columns display integer values with no decimals, but a non-integer
  # value (e.g. an averaged count) keeps one decimal so precision isn't lost.
  integer_columns <- c("missing", "Total wells", "Total targets")
  for(int_col in integer_columns){
    if(int_col %in% colnames(df2)){
      column_defs[[int_col]] <- colDef(cell = JS("
        function(cellInfo) {
          var v = cellInfo.value;
          if (v === null || v === undefined || v === '' || typeof v !== 'number') { return v; }
...
if(is.null(params$dataRuns)){
  ...
  if(is.null(params$dataDir)){
    ...
    # save xml file path
    xmlFilesPath <- file.path(params$dataDir, params$xmlFiles)
  }
  ...
  runs <- future_lapply(seq_along(xmlFilesPath), function(i) {
    loadNULISAseq(
      ...
      allowMissingCurveQuant = params$allowMissingCurveQuant
    )
  })
  for(i in 1:numFiles){
    ...
    if(length(runs[[i]]$zero_samples) != 0){
      ...
      # keep IC_normed columns aligned with Data so plateSummary()'s index-based
      # control CV (IC-normalized) is computed on the right wells (issue #655)
      if(!is.null(runs[[i]]$IC_normed$normData)) runs[[i]]$IC_normed$normData <- runs[[i]]$IC_normed$normData[,!(colnames(runs[[i]]$IC_normed$normData) %in% runs[[i]]$zero_samples), drop=FALSE]
      # drop excluded wells from the special-well name lists too, else a removed
      # IPC/NC/SC/Bridge well name lingers and interPlateNorm() indexes a
      # column that no longer exists in Data (issue #697)
      for(well_type in c("IPC", "NC", "SC", "Bridge", "Calibrator")){
        runs[[i]][[well_type]] <- runs[[i]][[well_type]][!(runs[[i]][[well_type]] %in% runs[[i]]$zero_samples)]
      }
    }
    ...
    if(length(runs[[i]]$IC_zero_samples) != 0){
      ...
      # keep IC_normed columns aligned with Data (issue #655; see above)
      if(!is.null(runs[[i]]$IC_normed$normData)) runs[[i]]$IC_normed$normData <- runs[[i]]$IC_normed$normData[,!(colnames(runs[[i]]$IC_normed$normData) %in% runs[[i]]$IC_zero_samples), drop=FALSE]
      # drop excluded wells from the special-well name lists too (issue #697; see above)
      for(well_type in c("IPC", "NC", "SC", "Bridge", "Calibrator")){
        runs[[i]][[well_type]] <- runs[[i]][[well_type]][!(runs[[i]][[well_type]] %in% runs[[i]]$IC_zero_samples)]
      }
    }

    # Recompute match_matrix after sample exclusion to keep indices in sync
    runs[[i]]$match_matrix <- calcSampleTargetNAs(
      runs[[i]]$targets$Curve_Quant,
      runs[[i]]$samples$SAMPLE_MATRIX
    )
...
plate_effect_single_plate <- FALSE
# set SC indicator
...
targetNoOutlierDetection <- lapply(seq_len(numFiles), function(i) {
  names(which(apply(runs[[i]]$Data[, runs[[i]]$NC, drop=FALSE] < 100, 1, all)))
})
...
# Build the intra-plate %CV summary table. Per-plate statistics are computed from
# the plate columns only, and the "Average" row aggregates those per-plate
# statistics (column means). Summarizing the pre-averaged per-target "Average"
# column instead would make the Average row's min/max/sd inconsistent with the
# plate rows it summarizes.
intra_cv_summary <- function(cv_matrix){
  plate_cols <- setdiff(colnames(cv_matrix), "Average")
  tbl <- column_summary_stats(cv_matrix[, plate_cols, drop=FALSE], n_digits=NULL)
  if("Average" %in% colnames(cv_matrix) && nrow(tbl) > 1){
    num_cols <- c("mean", "sd", "median", "min", "max")
    avg_row <- tbl[1, , drop=FALSE]
    avg_row[num_cols] <- as.list(colMeans(sapply(tbl[num_cols], as.numeric), na.rm=TRUE))
    # missing is the mean of the per-plate missing counts; kept as a decimal
    # since the mean of integer counts better describes average missingness
    avg_row[["missing"]] <- format(round(mean(as.integer(tbl[["missing"]]), na.rm=TRUE), 1), nsmall=1)
    rownames(avg_row) <- "Average"
...
IPC_intraCV_unnorm_summary_table <- intra_cv_summary(IPC_intraCV_unnorm)
IPC_intraCV_IC_summary_table <- intra_cv_summary(IPC_intraCV_IC)
if(indicatorSC==TRUE){
  SC_intraCV_unnorm_summary_table <- intra_cv_summary(SC_intraCV_unnorm)
  SC_intraCV_IC_summary_table <- intra_cv_summary(SC_intraCV_IC)
}
```
...
if(indicatorSC){
  ...
  SC_IPC_ratios <- lapply(runs, function(x){
    SC_means <- apply(x$IC_normed$normData[,x$SC, drop=FALSE], 1, mean, na.rm=TRUE)
    IPC_means <- apply(x$IC_normed$normData[,x$IPC, drop=FALSE], 1, mean, na.rm=TRUE)
...
if(indicatorSC){
  ...
  SC_NC_ratios <- lapply(runs, function(x){
    SC_means <- apply(x$IC_normed$normData[,x$SC, drop=FALSE], 1, mean, na.rm=TRUE)
    NC_means <- apply(x$IC_normed$normData[,x$NC, drop=FALSE], 1, mean, na.rm=TRUE)
...
detect_result <- detectability_summary(runs, exclude_targets = params$excludeTargets)

detectability_return_summary <- detect_result$run_summary
detectability_return_targets <- detect_result$run_targets
all_reverseCurve_targets <- detect_result$reverse_curve_targets
all_noDetect_nonRC_targets <- detect_result$noDetect_nonRC_targets
plasma_serum_groups <- detect_result$plasma_serum_groups

# if more than one plate, build Combined view for rendering
...
if(numFiles > 1){
  ...
  if (length(unique_types) <= 2){
    ...
    # Only filter out reverse curve targets (they get "High Abundance" rows appended below)
    combined_all_matrix <- combined_all_matrix[!(rownames(combined_all_matrix) %in% all_reverseCurve_targets), , drop = FALSE]

    # Add "High Abundance" for reverse curve targets
    if (length(all_reverseCurve_targets) > 0) {
      rc_matrix <- matrix("High Abundance", nrow = length(all_reverseCurve_targets), ncol = ncol(combined_all_matrix))
      rownames(rc_matrix) <- all_reverseCurve_targets
      colnames(rc_matrix) <- colnames(combined_all_matrix)
      combined_all_matrix <- rbind(combined_all_matrix, rc_matrix)
    }

    # Sort target rows alphabetically by name
    combined_all_matrix <- combined_all_matrix[order(tolower(rownames(combined_all_matrix))), , drop = FALSE]
...
if(params$reportType != "webApp"){
  dir.create(file.path(params$outfolder, "outputFiles"), recursive = TRUE)
}
```
...
if(multipleFiles){
  ...
  # Exclude IC controls and hidden targets from batch effect assessment
  # Non-RC noDetectability targets (rare case) are also excluded; reverse curve targets are kept
  batch_effect_noDetectability_targets <- all_noDetect_nonRC_targets
  ...
  # Guard: batch effect test requires at least 2 plates
  plate_effect_single_plate <- length(unique_plateEffectID) < 2
  if (plate_effect_single_plate) {
    single_plate_msg <- if (exists("plate_effect_group")) {
      paste0("Batch effect sample group '", plate_effect_group, "' only found on one plate. Batch effect test could not be completed. Change the plate_effect_group parameter to test for batch effects in other sample subgroups.")
    } else {
      "Only one plate found after filtering. Batch effect test could not be completed."
    }
    message(single_plate_msg)
  }
  ...
  if(n_significant_Ftest > 0 && all(is.na(plate_effect_sig_F_targets))){
    stop("The calculated Batch effects were all NA (check for identical XML input files)")
  }
  # drop NA/NULL-model targets before emmeans() (issue #655): a target whose
  # mixed model failed to fit (NULL) otherwise aborts the whole report
  plate_effect_sig_F_targets <- plate_effect_sig_F_targets[!is.na(plate_effect_sig_F_targets)]
  plate_effect_sig_F_targets <- plate_effect_sig_F_targets[!vapply(plate_effect_ftest$modelFits[plate_effect_sig_F_targets], is.null, logical(1))]
  n_significant_Ftest <- length(plate_effect_sig_F_targets)
  if(n_significant_Ftest > 0){
    pairwise_tests <- lapply(plate_effect_ftest$modelFits[plate_effect_sig_F_targets], function(x){
      ...
      for(p in 1:nrow(plate_effect_pct_sig)){
        # bounded match, not grep(plate, ...) which matched "Plate_1" in "Plate_10";
        # allow "(" / ")" since emmeans parenthesizes levels with special chars (issue #655)
        plate_pattern <- paste0("(^|[\\s(])", gsub("([.+*?^${}()|\\[\\]\\\\])", "\\\\\\1", plate_effect_pct_sig$plate[p]), "([\\s)]|$)")
        plate_contrasts_pval <- contrasts$sig_pval[grepl(plate_pattern, contrasts$contrast, perl=TRUE)]
    ...
    # if no significant targets, create a dummy pairwise test table using the
    # first non-NULL model fit (emmeans can't take NULL) (issue #655)
    first_valid_fit <- Filter(Negate(is.null), plate_effect_ftest$modelFits)
    if(length(first_valid_fit) == 0){
      # every model fit is NULL: emit an empty structure directly so the report
      # still renders instead of aborting in emmeans() (issue #655)
      pairwise_tests <- list(list(
        plate_effect_pct_sig = data.frame(plate=unique_plateEffectID,
                                          pct_sig_pval=NA, plate_effect=FALSE),
        contrasts = data.frame()))
    } else {
    pairwise_tests <- lapply(first_valid_fit[1], function(x){
      emm <- emmeans::emmeans(x, ~ plateEffectID)
      contrasts <- as.data.frame(pairs(emm))
      contrasts$sig_pval <- contrasts$p.value < params$plate_effect_pval_pairwise
      plate_effect_pct_sig <- data.frame(plate=unique_plateEffectID, pct_sig_pval=NA)
  ...
  for(i in 1:nrow(plate_effect_target_table)){
    ...
    # exact membership, not grepl(target, ...) which matched substrings ("CCL1"
    # in "CCL14") and regex metacharacters ("LTA|LTB") (issue #655)
    n_sig <- sum(vapply(strsplit(as.character(plate_effect_summary_table$sig_targets), ", "), function(x) target %in% x, logical(1)))
  ...
  raw_IPC_reads <- lapply(runs[plate_effect_PlateNames], function(x){
    apply(x$Data[!rownames(x$Data) %in% x$IC, x$IPC, drop=FALSE], 1, median, na.rm=TRUE)
  })
  ...
  if(!is.null(params$assayRunInfo)) {
    plate_effect_plate_table$instrument <- unlist(
      sapply(assayRunInfo[PlateNames %in% plate_effect_PlateNames],
             function(x) x[2]))
  }
  ...
  } # end if !plate_effect_single_plate
} # end if multipleFiles
```

```{r instrument-metadata-setup, include=FALSE, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate))}
if(is.null(params$assayRunInfo)){
  instrument_names <- NA
}
if(multipleFiles & !is.null(params$assayRunInfo)){
  # extract instrument data (filtered to plates included in batch effect test)
  plate_effect_assayRunInfo <- assayRunInfo[PlateNames %in% plate_effect_PlateNames]
  instruments <- unlist(sapply(plate_effect_assayRunInfo, function(x) x[2]))
  ...
  # create an instEffectID variable for PCA metadata
  sample_metadata$instEffectID <- NA
  for(i in 1:length(plate_effect_PlateNames)){
  ...
  # create binary instrument indicator columns for PCA
...
for(i in 1:numFiles){
  ...
  if(params$outputPlots && params$reportType != "webApp"){
    ...
    cat(html_content,
        file = file.path(params$outfolder, 'outputFiles', paste0(PlateNames[i], '_layout.html')))
  }
}
```
...
cat(paste0("- ", c("Parseable - Valid next generation sequencing (NGS) read (assay signal or background)",
                   ...
                   "Parseable Non-match - Valid NGS read (assay background)",
...
for(i in 1:numFiles){
  ...
  # Filter out excluded targets from qcTarget (IC, hidden, reverse curve, user-excluded)
  # Non-RC noDetectability targets (rare case) should appear in QC target table
  if(!is.null(runs[[i]]$qcTarget)){
    targets_to_exclude_qc <- c(
      runs[[i]]$IC,
      runs[[i]]$targets$targetName[which(runs[[i]]$targets$hide)],
      NULISAseqR:::get_reverse_curve_targets(runs[[i]]$targets)
    )
    if(!is.null(params$excludeTargets) && !is.null(params$excludeTargets[[i]])) {
...
for( i in 1:numFiles){
  ...
  sampleType_factor <- factor(runs[[i]]$samples$sampleType,
                              levels=c('Sample', 'IPC', 'SC', 'Bridge', 'NC'))
  well_order <- wellorder(runs[[i]]$samples,
                          sampleTypeFactor=sampleType_factor)
  # do sample QC
  qcSample <- QCFlagSample(runs[[i]]$Data, runs[[i]]$lod$aboveLOD, runs[[i]]$samples, runs[[i]]$targets, well_order, TAP=params$reportType=="internal", QCS=runs[[i]]$QCS, SN=runs[[i]]$SN)
  # default label set for the empty dotchart scaffold below; overwritten by
  # whichever of SC/Bridge relabeling actually runs (issue #697 review: `inds`
  # was otherwise only conditionally defined and could be undefined here)
  inds <- seq_len(nrow(qcSample))
  if(indicatorSC){
    inds <- which(grepl(paste(runs[[i]]$SC, collapse="|"), qcSample$sampleName))
    qcSample[inds, ]$sampleType <- "SC"
  }
  if(indicatorBridge && length(runs[[i]]$Bridge) > 0){
...
if(params$outQC){
  ...
  cat("\n\n")
  cat(tableWarningText(rep(TRUE, numTargetWarnings), 
                     textColor = "white", 
                     bgColor = "red", 
                     warningText = "Number of Targets with warnings"))
  cat("\n\n")
...
cat("## Detectability\n\n")
detect_desc <- paste0(
  "Target detectability is the percentage of samples that are above the limit of detection for that target. ",
  "A target is considered \"detectable\" if it is above limit of detection in greater than 50% of samples. ",
  "\"Overall\" detectability is the overall percentage of samples across all plates that are above LOD for a given target. ",
  "Target detectability reported here excludes ", CAL_IPC, ", ", AQSC_SC, ",", " & NC samples. ",
  "High abundance targets (reverse-curve quantified) are expected to be detectable and are excluded from detectability",
  if (length(all_reverseCurve_targets) > 0) paste0(" (", paste(all_reverseCurve_targets, collapse = ", "), ")") else "",
  ". ",
  "Rare case targets have individual detectability reported but are excluded from summary metrics",
  if (length(all_noDetect_nonRC_targets) > 0) paste0(" (", paste(all_noDetect_nonRC_targets, collapse = ", "), ")") else "",
  "."
)
cat(detect_desc, "\n\n")
printExcludedTargets(params$excludeTargets, PlateNames)
...
if(multipleFiles==FALSE & is.null(params$sampleGroupCovar)){
  ...
  # Remove RC targets before numeric rounding (they have "High Abundance" strings);
  # they will be re-appended after rounding
  detect_output_table <- detect_output_table[!(rownames(detect_output_table) %in% all_reverseCurve_targets), , drop = FALSE]
  ...
  # apply() drops to vector when nrow == 1; restore matrix structure
  if (!is.matrix(detect_output_table)) {
    detect_output_table <- matrix(detect_output_table, nrow = 1,
                                  dimnames = list(detect_output_table_rownames, names(detect_output_table)))
  } else {
...
detect_output_table <- detect_output_table[, !(colnames(detect_output_table) %in% colnames(detect_output_table)[grep('combined', tolower(colnames(detect_output_table)))]), drop = FALSE]

# Save numeric-only table for histograms before appending "High Abundance" strings
detect_hist_table <- detect_output_table

# Add "High Abundance" for reverse curve targets
if (length(all_reverseCurve_targets) > 0) {
  rc_targets_in_table <- all_reverseCurve_targets[!all_reverseCurve_targets %in% rownames(detect_output_table)]
  if (length(rc_targets_in_table) > 0) {
    rc_matrix <- matrix("High Abundance", nrow = length(rc_targets_in_table), ncol = ncol(detect_output_table))
    rownames(rc_matrix) <- rc_targets_in_table
    colnames(rc_matrix) <- colnames(detect_output_table)
    detect_output_table <- rbind(detect_output_table, rc_matrix)
  }
}

if(params$reportType != "webApp"){
  write.csv(detect_output_table, file.path(params$outfolder, 'outputFiles', 'target_detectability_table.csv'))
}
```
...
cat(paste0("Plot shows target NPQ distributions relative to LOD. For each target, LOD in NPQ units was subtracted from NPQ. Reverse-curve quantified targets are denoted by a \"*\" and are plotted as forward-curve targets for compatibility."), "\n\n")
par(mfrow=c(ceiling(numFiles/2), 2), mar=c(4,2,2,1))
for (i in 1:numFiles){
  # Create vector of targets to exclude: IC + hidden targets + reverse curve targets + user excluded targets
  # Non-RC noDetectability targets (rare case) should be plotted (they have LOD and individual detectability)
  ...
  targetBoxplot(data_matrix=IC_IPC$interNormData[[i]],
                title=PlateNames[i],
                subtractLOD=TRUE,
                blanks=runs[[i]]$NC,
                targetNoOutlierDetection=targetNoOutlierDetection[[i]],
...
if(params$reportType != "webApp"){
  # make directory -- we will put the file in the outputFiles directory
  dir.create(file.path(params$outfolder, "outputFiles"), recursive = TRUE)
  write.csv(SC_IPC_ratio_table, file.path(params$outfolder, "outputFiles", "SC_IPC_log2_ratio_table.csv"))
}
```
...
if(params$reportType != "webApp"){
  # make directory -- we will put the file in the outputFiles directory
  dir.create(file.path(params$outfolder, "outputFiles"), recursive = TRUE)
  write.csv(SC_NC_ratio_table, file.path(params$outfolder, "outputFiles", "SC_NC_log2_ratio_table.csv"))
}
```
...
if(params$reportType != "webApp"){
  write.csv(CV_output_table, file.path(params$outfolder, "outputFiles", "target_CV_table.csv"))
}
```
...
sampleboxplot <- function(data, 
  ...
  suppressWarnings({
    ...
    # add data axis label
    mtext(data_axis_label, side=data_axis_side, line=0.75, cex=0.5)
    # add data axis ticks and numbers
    axis(data_axis_side, cex.axis=0.5, tck=-0.01, mgp=c(3, 0.2, 0))
    # legend
...
for(i in 1:numFiles){
  ...
  sampleboxplot(sample_boxplot_data_norm, data_order=ordering, 
                title=paste0(PlateNames[i], ": NPQ"), 
                boxplot_colors=boxplot_colors2,
                label_colors=label_colors2,
                data_axis_label='NPQ')
  
  if(params$outputPlots && params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", paste0("sample_boxplot_", PlateNames[i], '.pdf')), width=4, height=9)
...
if(multipleFiles){
  ...
  if (plate_effect_single_plate) {
    cat(single_plate_msg, "\n\n")
  } else {
  # Report excluded non-RC noDetectability targets
  if(length(batch_effect_noDetectability_targets) > 0){
    cat(paste0("Note: The following noDetectability targets were excluded from batch effect assessment: ",
  ...
  } # end else !plate_effect_single_plate
}
```

```{r plate-effect-targets-table, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate)), results='asis', warning=FALSE}
if(multipleFiles){
  cat("\n\n---\n\n")
...
```{r random_plate-effect-icc_scatterplot, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate)), results='asis', out.width='75%', fig.asp=1, fig.align = 'center', warning=FALSE}
if(multipleFiles){
  cat("\n\n---\n\n")
...
```{r set-plate-test-boxplot-params, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate)), include=FALSE}
if(!multipleFiles){
  n_plot_cols <- 1
  n_plot_rows <- 1
...
```{r plate-effect-test-target-boxplots, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate)), results='asis', out.width='100%', fig.align = 'center',  fig.asp = tryCatch(figure_aspect, error = function(e) 0.75), warning=FALSE}
if(multipleFiles){
  cat("### Target boxplots\n\n")
...
```{r target-mean-by-plate-heatmap, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType != "webApp" & numFiles >= 3), results='asis', out.width='100%', fig.asp=3, fig.align = 'center', warning=FALSE}
if(multipleFiles){
...
```{r save-target-mean-by-plate-heatmap, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType=="internal" & numFiles >= 3), warning=FALSE, include=FALSE}
if(multipleFiles & params$reportType != "webApp"){
  ...
  pdf(file = file.path(params$outfolder, "outputFiles", "target_means_by_plate_heatmap.pdf"),
...
```{r pca-by-plate, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & numFiles >= 3), results='asis', out.width='70%', fig.asp=1, fig.align = 'center', warning=FALSE}
...
```{r plate-effect-sample-PCA, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType != "webApp"), results='asis', out.width='100%', fig.asp=1, fig.align = 'center', warning=FALSE, message=FALSE}
if(multipleFiles){
  cat("\n\n---\n\n")
  cat("### Sample PCA\n\n")
...
```{r plate-effect-sample-PCA-legend-setup, include=FALSE}
# Calculate legend aspect ratio based on label length, not just plate count.
# Long plate names reduce the effective number of columns that fit in the figure width.
pca_legend_ncol <- 5
pca_legend_fig_asp <- 0.1
if (exists("PlateNames") && length(PlateNames) > 0) {
  max_label_chars <- max(nchar(PlateNames))
  # Estimate columns that fit: ~80 chars width, each label needs ~(nchar + 4) chars for icon+padding
  pca_legend_ncol <- max(1, floor(80 / (max_label_chars + 4)))
  pca_legend_nrow <- ceiling(length(PlateNames) / pca_legend_ncol)
  pca_legend_fig_asp <- max(0.1, pca_legend_nrow * 0.1)
}
```

```{r plate-effect-sample-PCA-legend, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType != "webApp"), results='asis', out.width='100%', fig.asp=pca_legend_fig_asp, fig.align = 'center', warning=FALSE, message=FALSE}
if(multipleFiles){
  # plot PCA legend
  ...
  PCA_legend <- PCA_legend + ggplot2::guides(colour = ggplot2::guide_legend(ncol = pca_legend_ncol))
...
```{r plate-effect-sample-PCA-eigencorplot-fig-setup, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType != "webApp"), results='asis', fig.align = 'center', warning=FALSE, message=FALSE}
fig_height <- 1
if(params$outPlateEffect & multipleFiles){
  ...
  metavars <- unique(metavars)
  fig_height <- length(metavars) / 3 + 1
}
```

```{r plate-effect-sample-PCA-eigencorplot, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$reportType != "webApp"), results='asis', fig.height=fig_height, out.width='100%', fig.width=10, fig.align = 'center', warning=FALSE, message=FALSE}
if(params$outPlateEffect & multipleFiles){
...
```{r save-plate-effect-test-result, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$outputPlateEffectCSV==TRUE), include=FALSE}
if(multipleFiles & params$reportType != "webApp"){
  # make directory -- we will put the file in the outputFiles directory
  dir.create(file.path(params$outfolder, "outputFiles"), recursive = TRUE)
  
  # Batch effect test -- output CSV file
  write.csv(plate_effect_target_table, file.path(params$outfolder, "outputFiles", "target_plate_effect_table.csv"))
  ...
  qs2::qs_savem(sample_PCA, sample_PCA_eigencor_corvals, sample_PCA_eigencor_pvals,
                file=file.path(params$outfolder, "outputFiles", "batch_effect_test.qs2"))
}
```
...
for(i in 1:numFiles){
  ...
  # make plot
  if(params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", paste0("sample_QC_IC_median_", PlateNames[i], '.pdf')), width=12, height=4)
...
# intra-plate CV 
if(params$reportType != "webApp"){
  pdf(file.path(params$outfolder, "outputFiles", "intraCV_histogram.pdf"), 
      width=5, height=5)
...
if(multipleFiles==TRUE){
  if(params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", "interCV_histogram.pdf"), 
        width=5, height=5)
    colors <- alamarColorPalette(2, nReps = 5, palette = 2)
    par(mar=c(4.5, 4.5, 2, 1))
    if(indicatorSC==TRUE){
...
if(multipleFiles==FALSE & is.null(params$sampleGroupCovar)){
  ...
  detectability_data <- detect_hist_table
  detectability_data_name <- colnames(detect_hist_table)
  # make histogram
  if(params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", "target_detectability_histogram.pdf"),
        width=5, height=5)
    par(mar=c(4.5, 4.5, 2, 1))
...
if(multipleFiles==FALSE & !is.null(params$sampleGroupCovar)){
  ...
  if(params$reportType != "webApp"){
    filenames <- file.path(params$outfolder, "outputFiles", 
                       paste0("target_detectability_histogram_", sample_groups, '.pdf'))
...
if(multipleFiles==TRUE & is.null(params$sampleGroupCovar)){
  # get only Overall estimates
  detectability_data <- detect_hist_table[,grep('Overall', colnames(detect_hist_table))]
  detectability_data_name <- colnames(detect_hist_table)[grep('Overall', colnames(detect_hist_table))]
  # make histogram
  if(params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", "target_detectability_histogram.pdf"),
        width=5, height=5)
...
if(multipleFiles==TRUE & !is.null(params$sampleGroupCovar)){
  # get only Overall estimates
  detectability_data <- as.matrix(detect_hist_table[,grep('Overall', colnames(detect_hist_table))])
...
if(params$reportType != "webApp"){
  for(i in 1:numFiles){
    # Exclude IC + hidden targets + reverse curve targets
    pdf_boxplot_exclude_targets <- c(runs[[i]]$IC,
                                     runs[[i]]$targets$targetName[which(runs[[i]]$targets$hide)],
                                     NULISAseqR:::get_reverse_curve_targets(runs[[i]]$targets))
    pdf(file.path(params$outfolder, 'outputFiles', paste0('target_detectability_boxplot_', PlateNames[i], '.pdf')), width=20, height=7)
...
if(is.null(params$sampleGroupCovar) & numFiles > 1){
  ...
  if(params$reportType != "webApp"){
    pdf(file.path(params$outfolder, "outputFiles", "target_detectability_boxplot_all_plates.pdf"), width=20, height=7)
    # make plot
...
if(!is.null(params$sampleGroupCovar)){
  ...
  for (i in 1:length(params$xmlFiles)){
    ...
    runs[[i]]$samples$plate_sampleName <- paste0('plate_', i, '_', runs[[i]]$samples$sampleName)
    data_minus_LOD[[i]] <- sweep(plate_i_sample_data, MARGIN=1, STATS=plate_lod_IPC[[i]]$LOD_log2)
    # Exclude IC + hidden targets + reverse curve targets
  ...
  for(k in 1:length(sample_groups)){
    ...
    if (!is.null(colnames(sample_group_data))) {
      ...
      sample_group_filename <- file.path(params$outfolder, 'outputFiles', 
                                         paste0('target_detectability_boxplot_all_plates_', sample_groups[k], '.pdf'))
...
if(AbsData==TRUE & params$reportType != "webApp"){
  # 1.target QC plot
  pdf(file.path(params$outfolder, "outputFiles", "targetQC.pdf"), width=14, height=4)
  targetQCplot(runs)
  dev.off()
  ...
  if(!is.null(quant)){
    ...
    for(i in 1:length(subgroups)){
      pdf(file.path(params$outfolder, 'outputFiles', quant_hist_filenames[i]), width=5, height=5)
  ...
  pdf(file.path(params$outfolder, "outputFiles", "AQ_intraCV_histogram.pdf"), width=5, height=5)
  CV_AQ_Hist(CV_data = AQ_CV$average_intraCV$intraCV$SC, plot_type='intra')
  dev.off()
  
  if(multipleFiles){
    pdf(file.path(params$outfolder, "outputFiles", "AQ_interCV_histogram.pdf"), width=5, height=5)
...
if(params$reportType != "webApp"){
  ...
  qs2::qs_save(as.list(environment(), all.names = TRUE),
               file=file.path(params$outfolder, "outputFiles", "NULISAseq_QC_report.qs2")
  )
  rm(params)
}
```
...
if(fs::is_dir(tempDir)){
  files_to_delete <- c("outSamples.txt", "outTargets.txt", "outRunSummary.txt", 
                       "outCounts.txt", "outCounts_ICnormed.txt")
  unlink(file.path(tempDir, files_to_delete))
}
