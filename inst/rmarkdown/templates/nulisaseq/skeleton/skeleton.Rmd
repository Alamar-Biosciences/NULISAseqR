...
knitr::kable(QCSummary, caption='Number of samples that fail each QC Flag', 
             align='r', 
...
for( i in 1:length(runs)){
  ...
  # make plots
  # par(mfrow=c(ceiling(length(criteria$thresholds)/2), 2), mar=c(5,6,2,0.25))
  ...
  for(j in 1:length(names(criteria$thresholds))){
    ...
    dotchart(x=as.numeric(xval), 
             ...
             color=color, log=log)
    if(j==1){
      # need to loop over labels and color individualls
