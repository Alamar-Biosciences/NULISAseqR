...
for(i in 1:length(runs)){
  ...
  # make plot
  pdf(paste0('./figures/sample_QC_IC_median_', i, '.pdf'), width=4, height=9)
  par(mfcol=c(1, 1), mar=c(4,9,5,1))
  dotchart(x=as.numeric(xval), 
           xlab=xlab, 
           labels=rep('', length(labels)), las=1, 
           cex=0.7, xlim=c(min(c(xval, -30)), max(c(xval, 30))), 
           color=color)
  # need to loop over labels and color individuals
  for (k in 1:length(xval)){
    axis(side=2, at=k, 
  ...
  points(as.numeric(xvalPass),1:length(xvalPass), pch=19, col="green")
  points(as.numeric(xvalFail),1:length(xvalFail), pch=19, col="red")
  abline(v=-30, col='brown')
  abline(v=30, col='brown')
  abline(v=0, col='brown', lty=2)
  legend('bottomright', legend=c('Pass', "Fail"), col=c('green', 'red'), 
         pch=19, cex=0.7, bty='n', inset=c(0,1), xpd=T,horiz=T)
  title(main=paste0(PlateNames[i], ' IC_Median'), cex.main=1)
  dev.off()
}
...
for(i in 1:length(params$xmlFiles)){
  ...
  par(mar=c(4,6,2,0))
  sampleboxplot(mCherry_IPC$interNormData[[i]], ordering, "Sample distributions\nafter normalization", plate_sample_colors, boxplot_colors2)
  dev.off()
}
...
pdf('./figures/target_detectability_density.pdf',
    width=5, height=5)
par(mar=c(4.5, 4.5, 2, 1))
plot(density(detectability_targets[,'Overall'], na.rm=TRUE),
     main='Target detectability (%)',
     xlab='Detectability (%)', las=1)
x <- density(detectability_targets[,'Overall'], na.rm=TRUE)$x
x <- c(x[1], x, x[length(x)])
y <- c(0, density(detectability_targets[,'Overall'], na.rm=TRUE)$y, 0)
polygon(x, y,
        col=colors[[1]][5],
        border=colors[[1]][2])
abline(v=50, col=colors[[2]][2], lwd=2)
text(50, 0.013, '50%', pos=4, col=colors[[2]][2])
legend('topleft', c(paste0('mean = ', format(round(mean(detectability_targets[,'Overall'], na.rm=TRUE), 1), nsmall=1)), 
                     paste0('median = ', format(round(median(detectability_targets[,'Overall'], na.rm=TRUE), 1), nsmall=1))),
