---
params:
  study_name: "QC Report"
  assayName: !r NULL
  ## directory where xml files are located
  dataDir: "."
  ## XML file names
  #xmlFiles: !r c("Conc2_full.xml")
  xmlFiles: !r c("detectability_P1_Tr03.xml","detectability_P2_Tr03.xml")
  #xmlFiles: !r c("example_INF_QCS_SN.xml")
  #xmlFiles: !r c("example_CNS_QCS_SN.xml")
  # xmlFiles: !r c("detectability_P1_Tr03.xml")
  # xmlFiles: !r c("out.zip")
  dataRuns: NULL 
  ...
  assayRunInfo: !r NULL
  # assayRunInfo: !r list(list('Sept 3, 2024', 'ARGO Beta 2'), list('Sept 4, 2024', 'ARGO Beta 3'))
  ## type of report
  ## options:
  ## internal
  ## WebApp
  ...
  out_SC_IPC_Ratio: !r FALSE
  out_SC_NC_Ratio: !r FALSE
...
if(is.null(params$dataRuns)){
  ...
  tempDir <- tempdir() 
  for(i in 1:numFiles){
    if(tools::file_ext(xmlFilesPath[i]) == "zip"){
      fileList <- unzip(xmlFilesPath[i], exdir = tempDir, overwrite = TRUE)
      runs[[i]]$samples <- read.table(paste0(tempDir, "/outSamples.txt"), header = TRUE, sep = "\t", comment.char="")
      runs[[i]]$targets <- read.table(paste0(tempDir, "/outTargets.txt"), header = TRUE, sep = "\t", comment.char="")
      runs[[i]]$SampleNames <- runs[[i]]$samples$sampleNames
      
      runs[[i]]$Data <- as.matrix(read.table(paste0(tempDir, "/outCounts.txt"), header = TRUE, sep = "\t", comment.char="", row.names=1))
      runs[[i]]$AQ <- as.matrix(read.table(paste0(tempDir, "/outCounts_AQ.txt"), header = TRUE, sep = "\t", comment.char="", row.names=1))
      runs[[i]]$IC_normed$normData <- as.matrix(read.table(paste0(tempDir, "/outCounts_ICnormed.txt"), header = TRUE, sep = "\t", comment.char="", row.names=1))
      runs[[i]]$IC <- "mCherry"
      runs[[i]]$IPC <- runs[[i]]$samples$sampleName[which(runs[[i]]$samples$type == "IPC")]
      runs[[i]]$NC <- runs[[i]]$samples$sampleName[which(runs[[i]]$samples$type == "NC")]
      runs[[i]]$SC <- runs[[i]]$samples$sampleName[which(runs[[i]]$samples$type == "SC")]
      runs[[i]]$Bridge <- runs[[i]]$sampleName[which(runs[[i]]$samples$type == "Bridge")]
      runs[[i]]$Calibrator <- runs[[i]]$samples$sampleName[which(runs[[i]]$samples$type == "Calibrator")]
      runSummary <- read.table(paste0(tempDir,"/outRunSummary.txt"), header=FALSE, stringsAsFactor=FALSE, na.strings="", comment.char="")
      runs[[i]]$RunSummary <- list()
      for (j in 1:nrow(runSummary)){
        runs[[i]]$RunSummary[runSummary[j, 1]] <- as.numeric(runSummary[j, 2])
      }
      runs[[i]]$targets$targetType <- rep("Target", length(runs[[i]]$targets$targetName))
      runs[[i]]$targets$targetType[grep(paste(runs[[i]]$IC, collapse="|"), runs[[i]]$targets$targetName)] <- "Control"
      
      runs[[i]]$samples$sampleType <- unlist(lapply(runs[[i]]$samples$type, FUN=function(t) gsub(pattern="sample", replacement="Sample", x=t, fixed=T)))
      
    }else{
      runs[[i]] <- loadNULISAseq(xmlFilesPath[i], plateID=PlateNames[i], 
    ...
    zero_samples <- names(which(colSums(runs[[i]]$Data == 0) == nrow(runs[[i]]$Data)))
    if(length(zero_samples) != 0){
      runs[[i]]$samples <- runs[[i]]$samples[!(runs[[i]]$samples$sampleName %in% zero_samples),]
      runs[[i]]$SampleNames <- runs[[i]]$SampleNames[!(runs[[i]]$SampleNames %in% zero_samples)]
      runs[[i]]$Data <- runs[[i]]$Data[,!(colnames(runs[[i]]$Data) %in% zero_samples)]
    }
    
    # check and exclude samples with 0 IC reads
    mCherry_zero_samples <- colnames(runs[[i]]$Data)[runs[[i]]$Data[params$IC, ] == 0]
    if(length(mCherry_zero_samples) != 0){
      runs[[i]]$samples <- runs[[i]]$samples[!(runs[[i]]$samples$sampleName %in% mCherry_zero_samples),]
      runs[[i]]$SampleNames <- runs[[i]]$SampleNames[!(runs[[i]]$SampleNames %in% mCherry_zero_samples)]
      runs[[i]]$Data <- runs[[i]]$Data[,!(colnames(runs[[i]]$Data) %in% mCherry_zero_samples)]
    }
...
if(any(is.na(PlateNames)) | any(duplicated(PlateNames))) {
  PlateNames <- paste0('Plate ', formatC(1:numFiles, width=2, format='d', flag='0'))
  if(numFiles > 99) PlateNames <- paste0('Plate ', formatC(1:numFiles, width=3, format='d', flag='0'))
...
if(params$sort_by_plateNames){
  runs <- runs[order(PlateNames)]
  runSummaries <- runSummaries[order(PlateNames)]
  read_summary <- read_summary[,order(PlateNames)]
  PlateNames <- sort(PlateNames)
}
...
targetNoOutlierDetection <- vector('list', numFiles)
for (i in 1:numFiles){
  targetNoOutlierDetection[[i]] <- names(which(apply(runs[[i]]$Data[, runs[[i]]$NC] < 100, 1, all)))
  plate_lod[[i]] <- lod(data_matrix=mCherry_IPC$interNormData[[i]], 
                        blanks=runs[[i]]$NC, 
                        min_count=0,
...
blank_outlier_table <- lapply(plate_lod, function(x) x$blank_outlier_table)
for (i in 1:numFiles){
  if(!is.null(blank_outlier_table[[i]])){
    if(nrow(blank_outlier_table[[i]]) > 0){
      blank_outlier_table[[i]]$PlateID <- PlateNames[i]
      blank_outlier_table[[i]] <- blank_outlier_table[[i]][,c(5,1:4)]
      rownames(blank_outlier_table[[i]]) <- NULL
    }
  }
}
blank_outlier_table <- blank_outlier_table[!sapply(blank_outlier_table, is.null)]
blank_outlier_table <- do.call(rbind, blank_outlier_table)
```
...
# this function is used to summarize CV so any NA is actually missing not zeros
...
if(indicatorSC){
  ...
  if(numFiles > 1){
    SC_IPC_ratio_matrix <- Reduce(cbind, SC_IPC_ratios)
    colnames(SC_IPC_ratio_matrix) <- PlateNames
    # remove ICs
    SC_IPC_ratio_table <- SC_IPC_ratio_matrix[!(rownames(SC_IPC_ratio_matrix) %in% params$IC),]
    ...
    SC_IPC_ratio_table <- SC_IPC_ratios[[1]][!(names(SC_IPC_ratios[[1]]) %in% params$IC)]
...
if(numFiles > 1){
  ...
  detectability_return_targets[["Overall"]] <- format(round(sum_matrix, 1), nsmall=1)
...
pandoc.p(paste0("Shows the layout of samples, controls (e.g., negative controls (NC), ", CAL_IPC_full, " and special wells if any (e.g., ", AQSC_SC_full, ") on the 96 well assay plate."))

if(!is.null(PlateNameMessage)) pandoc.p(PlateNameMessage)
...
for(i in 1:numFiles){
  ...
  for (j in 1:length(well_types)){
    ...
    if(length(inds) > 0){
      if(AbsData){
        val[inds] <- gsub("^IPC_([1234])$", "CAL_\\1", val[inds])
        val[inds] <- gsub("^SC_([1234])$", "AQSC_\\1", val[inds])
      }
  ...
  cat(kableExtra::kbl(val, caption=paste0(PlateNames[i], ": Plate Layout"), 
...
pandoc.p("Each sample of the assay is spiked with the same concentration of an internal control (IC). This is used both as part of a well normalization procedure and as a method for assessing the uniformity of the assay run. SD: standard deviation. CV: coefficient of variation") 
if(length(params$IC) == 1){
  ...
  cat(knitr::kable(IC_tables, 
                   caption="Internal Control (IC) summary", 
                   align='r', 
  ...
  for(i in 1:numFiles){
    ...
    cat(knitr::kable(runSummaries[[i]]$IC_table, 
                     caption=paste0(PlateNames[i], ": IC summary"), 
                     align='r', 
                     format.args=list(big.mark = ","), 
...
```{r, eval=(params$outReadSummary), results='asis'}
knitr::asis_output("\n\n---\n\n")
pandoc.header(CAL_IPC_full, 3)
pandoc.p(paste0(CAL_IPC, " samples are pooled plasma controls used both to normalize samples between experiments and to assess performance (e.g., precision) of the assay."))
pandoc.p("Note: %CV in this table does not exclude values below LOD. See Coefficient of variation section for %CV (before and after normalization) that excludes values below LOD.")
for(i in 1:numFiles){
  ...
  cat(knitr::kable(runSummaries[[i]]$IPC_table, caption=paste0(PlateNames[i], ": ", CAL_IPC, " summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
...
for(i in 1:numFiles){
  ...
  cat(knitr::kable(runSummaries[[i]]$NC_table, caption=paste0(PlateNames[i], ": NC summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
...
knitr::asis_output("\n\n---\n\n")
pandoc.header(AQSC_SC_full, 3)
pandoc.p(paste0(AQSC_SC, " samples are pooled plasma controls from an independent pooled plasma source that is different from ", CAL_IPC, "s. ", AQSC_SC, "s are used for calculating intra- and inter-plate coefficient of variation (CV). They are used to obtain an unbiased measure of inter-plate CV when ", CAL_IPC, " normalization is used."))
for(i in 1:numFiles){
  table.attr <- paste0("id=\"SC-summary", i, "\"")
  if(AbsData){
    rownames(runSummaries[[i]]$SC_table) <- gsub("^SC_([1234])$", "AQSC_\\1", rownames(runSummaries[[i]]$SC_table))
  }
  cat(knitr::kable(runSummaries[[i]]$SC_table, caption=paste0(PlateNames[i], ": ", AQSC_SC, " summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
...
pandoc.p("Bridge samples can be used for inter-plate normalization. Bridge sample normalization uses the bridge sample subset to estimate median target-specific differences in NPQ, and then uses these NPQ differences to normalize all samples in a manner similar to intensity normalization.")
for(i in 1:numFiles){
  if(!is.null(runSummaries[[i]]$Bridge_table)){
    table.attr <- paste0("id=\"Bridge-summary", i, "\"")
    cat(knitr::kable(runSummaries[[i]]$Bridge_table, caption=paste0(PlateNames[i], ": Bridge summary"), 
                     align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
          kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
  }
}
```
...
pandoc.list( qc_criteria)
pandoc.p(paste0("QC criteria are assessed either on unnormalized (raw) or ", CAL_IPC, "-normalized (", CAL_IPC, ") data."))
...
for( i in 1:numFiles){
  ...
  cat(knitr::kable(qcPlate[,1:5], 
                   caption=paste0(PlateNames[i], ": Plate QC Summary"), 
                   align='r', 
        ...
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
        row_spec(which(qcPlate$status == "Warning"), bold=T, color="white", background="#ebc634"))
}
```
...
```{r, eval=(params$outQC & params$reportType!="internal"), out.width='100%', results='asis'}
qcText <- QCSampleCriteria(TAP=FALSE)
items <- paste0(qcText$properNames, " - ", qcText$explanations)
pandoc.list(items)

if(length(zero_samples) != 0) {
  pandoc.p(paste0("Sample Excluded: ", paste(zero_samples, collapse = ", "), " (Sample with zero value for all targets are excluded.)"))
}

if(length(mCherry_zero_samples) != 0) {
  pandoc.p(paste0("Sample Excluded: ", paste(mCherry_zero_samples, collapse = ", "), " (Sample with zero IC reads are excluded.)"))
}
```
...
for (i in 1:numFiles){
  ...
  if(AbsData){
    qcTarget <- QCFlagTarget(runs[[i]]$AQ$Data_AQ_aM, runs[[i]]$targets, runs[[i]]$samples, runs[[i]]$AQ$targetAQ_param$SC_conc)
  }
  ...
  if(AbsData){
    for (j in 1:length(Tcriteria$thresholdNames)){
      ...
      indsT <- which(startsWith(names(Tcriteria$thresholdNames)[j], qcTarget$flagName))  
      QC_criterion_Target_j_data <- qcTarget[indsT, ]
      ...
      }else if(names(Tcriteria$thresholdNames)[j] == "Target_Conc_CV"){
...
# add percentages
QCSummary[,2:ncol(QCSummary)] <- apply(QCSummary[,2:ncol(QCSummary)], 2, function(x){
  pct <- paste0(x, ' (', format(round(x / QCSummary[,1] * 100, 1), nsmall=1), '%)')
  return(pct)
})

if(params$highlight_TAP_report_fields==TRUE){
  # print sample QC summary table
  knitr::kable(QCSummary, caption=paste0('Number (%) of samples that have warnings for each QC criterion (excludes ', CAL_IPC, ", ", AQSC_SC, ' & NC samples)'),
               align='r',
               table.attr="id=\"QC-summary-table\"",
               format.args=list(big.mark = ",", digits=2, escape=FALSE)) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
    row_spec(1:nrow(QCSummary), bold=FALSE, color="black", background="yellow")
} else if(params$highlight_TAP_report_fields==FALSE){
  # print sample QC summary table
  knitr::kable(QCSummary, caption=paste0('Number (%) of samples that have warnings for each QC criterion (excludes ', CAL_IPC, ", ", AQSC_SC, ' & NC samples)'),
               align='r',
               table.attr="id=\"QC-summary-table\"",
...
for (i in 1:numFiles){
  ...
  for (j in 1:length(criteria$thresholdNames)){
    ...
    if(length(QC_fail_samples_j) > 0){
      QC_sample_fail[[j]] <- rbind(QC_sample_fail[[j]], data.frame(Plate=PlateNames[i], Samples=QC_fail_samples_j, val=QC_fail_samples_j_vals))
    }
  }
}
...
for(i in 1:length(names(criteria$thresholdNames))){
  if(nrow(QC_sample_fail[[i]]) > 0){
    ...
    if(names(criteria$thresholdNames)[i]=='IC_Median' & params$highlight_TAP_report_fields==TRUE){
      cat(knitr::kable(QC_sample_fail[[i]], caption=paste0(tableMsg, criteria$properNames)[i],
                       align='r',
                       table.attr="id=\"sample-QC-summary-table\"",
                       format.args=list(big.mark = ",", digits=2, escape=FALSE)) %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
            row_spec(1:nrow(QC_sample_fail[[i]]), bold=FALSE, color="black", background="yellow"))
    } else {
      cat(knitr::kable(QC_sample_fail[[i]], caption=paste0(tableMsg, criteria$properNames)[i],
                       align='r',
                       table.attr="id=\"sample-QC-summary-table\"",
...
# pandoc.p("Note that pass / fail status of SN QC criterion is: SN/QCS < threshold1 AND QCS > threshold2. Values shown are SN/QCS.")
...
```{r, eval=(params$outQC & AbsData), out.width='100%', results='asis'}
pandoc.header("Target QC", 3)
pandoc.header("Target QC summary tables", 4)
pandoc.p("Target-specific QC criteria:")
qcText <- QCTargetCriteria()
items <- paste0(qcText$properNames, " - ", qcText$explanations)
pandoc.list(items)
```

```{r, eval=(params$outQC & AbsData), out.width='100%', results='asis'}
# table to show which specific Targets fail each QC flag
QC_target_fail <- vector(mode='list', length=length(names(Tcriteria$thresholdNames)))
names(QC_target_fail) <- names(Tcriteria$thresholdNames)
QC_target_fail <- lapply(QC_target_fail, function(x) x <- data.frame(Plate=character(0), QC_fail_targets=character(0)))
for (i in 1:numFiles){
  qcTarget <- QCFlagTarget(runs[[i]]$AQ$Data_AQ_aM, runs[[i]]$targets, runs[[i]]$samples, runs[[i]]$AQ$targetAQ_param$SC_conc)
  qcTarget <- qcTarget[which(qcTarget$status != "NA"),]
  for (j in 1:length(Tcriteria$thresholdNames)){
    QC_criterion_j_data <- qcTarget[qcTarget$flagName == names(Tcriteria$thresholdNames)[j], ]
    QC_criterion_j_data_targets <- QC_criterion_j_data[QC_criterion_j_data$target %in% runs[[i]]$targets$targetName,]
    QC_fail_targets_j <- QC_criterion_j_data_targets$target[which(QC_criterion_j_data_targets$status==TRUE & !is.na(QC_criterion_j_data_targets$status))]
    QC_fail_targets_j_vals <- QC_criterion_j_data_targets$val[which(QC_criterion_j_data_targets$status==TRUE & !is.na(QC_criterion_j_data_targets$status))]
    if(length(QC_fail_targets_j) > 0){
      QC_target_fail[[j]] <- rbind(QC_target_fail[[j]], data.frame(Plate=PlateNames[i], Targets=QC_fail_targets_j, val=100*as.numeric(QC_fail_targets_j_vals)))
    }
  }
}
...
for(i in 1:length(names(Tcriteria$thresholdNames))){
  if(nrow(QC_target_fail[[i]]) > 0){
    tableMsg = "Targets with warnings: "
    if(names(Tcriteria$thresholdNames)[i]=='Target_Conc_Accuracy' & params$highlight_TAP_report_fields==TRUE){
      cat(knitr::kable(QC_target_fail[[i]], caption=paste0(tableMsg, names(Tcriteria$thresholdNames)[i]),
                       align='r',
                       table.attr="id=\"target-QC-summary-table\"",
                       format.args=list(big.mark = ",", digits=2, escape=FALSE)) %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
            row_spec(1:nrow(QC_target_fail[[i]]), bold=FALSE, color="black", background="yellow"))
    } else {
      cat(knitr::kable(QC_target_fail[[i]], caption=paste0(tableMsg, names(Tcriteria$thresholdNames)[i]),
                       align='r',
                       table.attr="id=\"target-QC-summary-table\"",
...
if (combine_detectability){
  ...
  if(params$highlight_TAP_report_fields==TRUE){
    cat(knitr::kable(detectability_return_summary[["Combined"]],
          ...
          kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                        full_width = TRUE) %>%
          row_spec(1:nrow(detectability_return_summary[["Combined"]]), bold=FALSE, color="black", background="yellow"))
  } else {
    cat(knitr::kable(detectability_return_summary[["Combined"]],
                     caption=NULL, 
                     align="r", table.attr=table.attr,
  ...
  for (i in names(detectability_return_summary)){
    ...
    if(params$highlight_TAP_report_fields==TRUE & i=='Overall'){
      cat(knitr::kable(detectability_return_summary[[i]],
            ...
            kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                          full_width = TRUE) %>% 
            row_spec(1:nrow(detectability_return_summary[[i]]), bold=FALSE, color="black", background="yellow"))
    } else {
      cat(knitr::kable(detectability_return_summary[[i]],
                       caption=paste(as.character(i)), 
                       align="r", table.attr=table.attr,
...
```{r, results='asis', eval=(params$outDetectability), out.width='100%', fig.asp=detect_fig_asp}
pandoc.header("Target detectability table", 3)
...
pandoc.p(paste0("Plot shows target IC-", CAL_IPC, "-normalized count distributions relative to LOD. For each target, log2(LOD) was subtracted from log2(count) for IC-normalized data."))
...
```{r, eval=(params$outIntraPlateNorm & indicatorSC==TRUE), results='asis'}
knitr::asis_output("\n\n---\n\n")
pandoc.header("Coefficient of variation", 2) 
pandoc.header("Intra-plate CV summary", 3)
pandoc.p(paste0("%CV was calculated for each target (excluding internal controls) for the ", AQSC_SC, "s and ", CAL_IPC, "s. Only values above LOD were used to calculate %CV. Some targets may have missing CV due to too many values below LOD."))
pandoc.header("Intra-plate CV table -- unnormalized", 4)
knitr::kable(SC_intraCV_unnorm_summary_table, caption=paste0(AQSC_SC, ' intra-plate %CV -- unnormalized'), align='r', table.attr="id=\"SC-intra-plateCV-unnorm-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)

knitr::kable(IPC_intraCV_unnorm_summary_table, caption=paste(CAL_IPC, ' intra-plate %CV -- unnormalized'), align='r', table.attr="id=\"IPC-intra-plateCV-unnorm-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)

pandoc.header("Intra-plate CV table -- IC-normalized", 4)
if(params$highlight_TAP_report_fields==TRUE){
  knitr::kable(SC_intraCV_mCherry_summary_table, caption=paste0(AQSC_SC, ' intra-plate %CV -- IC-normalized'), align='r', table.attr="id=\"SC-intra-plateCV-mCherry-summary\"",
               format.args=list(big.mark = ",", digits=3)) %>% 
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
    row_spec(1:nrow(SC_intraCV_mCherry_summary_table), bold=FALSE, color="black", background="yellow")
} else {
  knitr::kable(SC_intraCV_mCherry_summary_table, caption=paste0(AQSC_SC, ' intra-plate %CV -- IC-normalized'), align='r', table.attr="id=\"SC-intra-plateCV-mCherry-summary\"",
               format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) 
}

knitr::kable(IPC_intraCV_mCherry_summary_table, caption=paste0(CAL_IPC, ' intra-plate %CV -- IC-normalized'), align='r', table.attr="id=\"IPC-intra-plateCV-mCherry-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)
```

```{r, out.width='100%', eval=(params$outIntraPlateNorm & indicatorSC == TRUE), fig.asp=(0.05*numFiles + 0.25), results='asis'}
pandoc.header("Intra-plate CV boxplots by plate", 4)
...
colnames(SC_intraCV_mCherry2) <- paste0(colnames(SC_intraCV_mCherry), ' IC-normalized')
...
```{r, out.width='100%', eval=(params$outIntraPlateNorm), fig.asp=(0.05*numFiles + 0.25)}
...
colnames(IPC_intraCV_mCherry2) <- paste0(colnames(IPC_intraCV_mCherry), ' IC-normalized')
...
knitr::asis_output("\n\n---\n\n")
pandoc.header("Inter-plate CV summary", 3)
...
```{r eval=(multipleFiles & params$outInterPlateNorm), results='asis'}
pandoc.header("Inter-plate CV table", 4)
...
if(indicatorSC==TRUE){
  if(params$highlight_TAP_report_fields==TRUE){
    knitr::kable(SC_interCV_summary_table2, 
                 caption=paste0(AQSC_SC, " inter-plate CV"), 
                 align='r', 
                 table.attr="id=\"SC-inter-plateCV-summary\"",
                 format.args=list(big.mark = ",", digits=3)) %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
      row_spec(which(rownames(SC_interCV_summary_table2)==paste0('IC + ', CAL_IPC)), bold=FALSE, color="black", background="yellow")
  } else {
    knitr::kable(SC_interCV_summary_table2, 
                 caption=paste0(AQSC_SC, " inter-plate CV"), 
                 align='r', 
...
knitr::kable(IPC_interCV_summary_table2, 
             caption=paste0(CAL_IPC, ' inter-plate CV'), 
             align='r', 
...
```{r out.width='100%', fig.asp=0.35, eval=(params$outInterPlateNorm & multipleFiles & indicatorSC), results='asis'}
pandoc.header("Inter-plate CV boxplot", 4)
...
```{r, eval=(params$outIntraPlateNorm & indicatorSC==TRUE), results='asis'}
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0("Target ", AQSC_SC, " intra-plate CV table"), 3)
pandoc.p("Targets with Overall (across-plate average) intra-plate CV > 30% are highlighted in red.")

table.attr <- paste0("id=\"intraCV-SC-IC-norm", "\"")
cat(knitr::kable(SC_intraCV_mCherry,
                 caption=paste0(AQSC_SC, " intra-plate CV, IC-normalized"), 
                 align="r", table.attr=table.attr,
                 format.args=list(big.mark = ",", digits=2)) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                    full_width = TRUE)  %>%
      row_spec(which(SC_intraCV_mCherry[,ncol(SC_intraCV_mCherry)] > 30), bold=T, color="white", background="red"))
```


```{r, eval=(params$outInterPlateNorm & multipleFiles & indicatorSC), results='asis'}
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0("Target ", AQSC_SC, " inter-plate CV table"), 3)
pandoc.p(paste0("Targets with IC + ", CAL_IPC, " inter-plate CV > 30% are highlighted in red."))
IC_IPC <- which(colnames(SC_interCV_results)== "IC + IPC")
if(AbsData){
  colnames(SC_interCV_results) <- gsub(" IPC$", " CAL", colnames(SC_interCV_results))
}
table.attr <- paste0("id=\"interCV-SC", "\"")
cat(knitr::kable(SC_interCV_results,
                 caption=paste0(AQSC_SC, " inter-plate CV"), 
                 align="r", table.attr=table.attr,
                 format.args=list(big.mark = ",", digits=2)) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                    full_width = TRUE) %>%
      row_spec(which(SC_interCV_results[, IC_IPC] > 30), bold=T, color="white", background="red"))
```


```{r, eval=(params$output_SC_IPC_Ratio==TRUE & indicatorSC), include=FALSE}
# make directory -- we will put the file in the outputFiles directory
dir.create("./outputFiles")

# SC IPC log2 ratio -- output CSV file
write.csv(SC_IPC_ratio_table, './outputFiles/SC_IPC_log2_ratio_table.csv')
```

```{r, eval=(params$output_SC_NC_Ratio==TRUE & indicatorSC), include=FALSE}
# make directory -- we will put the file in the outputFiles directory
dir.create("./outputFiles")

# SC NC log2 ratio -- output CSV file
write.csv(SC_NC_ratio_table, './outputFiles/SC_NC_log2_ratio_table.csv')
```


```{r, eval=params$outputCoefVarCSV==TRUE, include=FALSE}
# make directory -- we will put the file in the outputFiles directory
dir.create("./outputFiles")
...
if(indicatorSC & multipleFiles){
  ...
  colnames(SC_intraCV_mCherry) <- paste0('SC_intraCV_IC_norm_', colnames(SC_intraCV_mCherry))
  colnames(IPC_intraCV_unnorm) <- paste0('IPC_intraCV_unnorm_', colnames(IPC_intraCV_unnorm))
  colnames(IPC_intraCV_mCherry) <- paste0('IPC_intraCV_IC_norm_', colnames(IPC_intraCV_mCherry))
  colnames(SC_intraCV_unnorm) <- gsub(' ', '_', colnames(SC_intraCV_unnorm))
  colnames(SC_intraCV_mCherry) <- gsub(' ', '_', colnames(SC_intraCV_mCherry))
  colnames(IPC_intraCV_unnorm) <- gsub(' ', '_', colnames(IPC_intraCV_unnorm))
  colnames(IPC_intraCV_mCherry) <- gsub(' ', '_', colnames(IPC_intraCV_mCherry))
  # merge intra CV
  intraCV_list <- list(SC_intraCV_unnorm, SC_intraCV_mCherry, IPC_intraCV_unnorm, IPC_intraCV_mCherry)
...
if(!indicatorSC & multipleFiles){
  ...
  colnames(IPC_intraCV_mCherry) <- gsub(' ', '_', colnames(IPC_intraCV_mCherry))
  # merge intra CV
  intraCV_list <- list(IPC_intraCV_unnorm, IPC_intraCV_mCherry)
  intraCV_table <- Reduce(merge, lapply(intraCV_list, function(x) data.frame(x, rn = row.names(x))))
  rownames(intraCV_table) <- intraCV_table[,1]
  intraCV_table <- intraCV_table[,2:ncol(intraCV_table)]
  # set interCV colnames
...
write.csv(CV_output_table, './outputFiles/target_CV_table.csv')
```
...
pandoc.p(paste0("Sample boxplots show distributions of the log2 counts for each sample, unnormalized and after IC + ", CAL_IPC, " normalization."))
...
for(i in 1:numFiles){
  ...
  if(AbsData){
    colnames(mCherry_IPC$interNormData[[i]]) <- gsub("^IPC_([1234])$", "CAL_\\1", colnames(mCherry_IPC$interNormData[[i]]))
    colnames(mCherry_IPC$interNormData[[i]]) <- gsub("^SC_([1234])$", "AQSC_\\1", colnames(mCherry_IPC$interNormData[[i]]))
  }
...
for(i in 1:numFiles){
  ...
  if(AbsData){
    colnames(corrvals$corr) <- gsub("^IPC_([1234])$", "CAL_\\1", colnames(corrvals$corr))
    colnames(corrvals$corr) <- gsub("^SC_([1234])$", "AQSC_\\1", colnames(corrvals$corr))
    rownames(corrvals$corr) <- gsub("^IPC_([1234])$", "CAL_\\1", rownames(corrvals$corr))
    rownames(corrvals$corr) <- gsub("^SC_([1234])$", "AQSC_\\1", rownames(corrvals$corr))
  }
...
pandoc.p(paste0("Hierarchical clustering of both samples and targets was done using a Euclidean distance metric and complete linkage. Data is log2 and IC-", CAL_IPC, " normalized; targets are centered and scaled."))
# define colors
paletteLength <- 50
...
for(i in 1:numFiles){
  ...
  if(AbsData){
    rownames(data_mCherry_IPC_log2_scaled2) <- gsub("^IPC_([1234])$", "CAL_\\1", rownames(data_mCherry_IPC_log2_scaled2))
    rownames(data_mCherry_IPC_log2_scaled2) <- gsub("^SC_([1234])$", "AQSC_\\1", rownames(data_mCherry_IPC_log2_scaled2))
  }
...
for(i in 1:numFiles){
  ...
  val <- gsub('_', ' ', val) # replace underscore with space for text wrapping
  table.attr <- paste0("id=\"plate-summary", i, "\"")
  kableExtra::kbl(val, caption=paste0(PlateNames[i], ": Plate Layout"), 
                  align="c", escape=FALSE, table.attr=table.attr) %>%
    kable_styling(font_size=9, bootstrap_options = c("striped", "hover", "condensed"), 
                  full_width = TRUE) %>%
    column_spec(column=1:13, width = "1.75cm", border_left = TRUE, border_right = TRUE) %>%
    row_spec(0:8, extra_css = "border-bottom: 1px solid")  %>%
    save_kable(file=paste0('./outputFiles/', PlateNames[i], '_layout.html'))
}
...
for(i in 1:numFiles){
  ...
  # make plot
  pdf(paste0('./outputFiles/sample_QC_IC_median_', PlateNames[i], '.pdf'), width=12, height=4)
...
for(i in 1:numFiles){
  ...
  ordering <- rev(ordering)
  # make plots
  pdf(paste0('./outputFiles/sample_boxplot_', PlateNames[i], '.pdf'), width=4, height=9)
  par(mar=c(4,6,2,0))
  sampleboxplot(mCherry_IPC$interNormData[[i]], data_order=ordering, 
...
if(indicatorSC==TRUE){
  intraCV_mean <- SC_intraCV_mCherry[,ncol(SC_intraCV_mCherry)]
} else {
  intraCV_mean <- IPC_intraCV_mCherry[,ncol(IPC_intraCV_mCherry)]
}
...
##########################
for(i in 1:numFiles){
  pdf(paste0('./outputFiles/target_detectability_boxplot_', PlateNames[i], '.pdf'), width=20, height=7)
  targetBoxplot(mCherry_IPC$interNormData[[i]],
...
if(is.null(params$sampleGroupCovar) & numFiles > 1){
  ...
  for (i in 1:length(params$xmlFiles)){
    ...
    data_minus_LOD[[i]] <- data_minus_LOD[[i]][!rownames(data_minus_LOD[[i]]) %in% runs[[i]]$IC,]
  }
...
```{r, include=FALSE, eval=params$outputRData==TRUE}
# Save R Data file
# Rename the param variable to avoid conflict when reading multiple RData files
dir.create("./outputFiles")
input_params <- params
rm(params)
save(list=ls(all.names = TRUE),
     file='./outputFiles/NULISAseq_QC_report.RData',
     envir = environment())
```
