...
for( i in 1:length(runs)){
  ...
  qcPlate$status[which(qcPlate$status == TRUE)] <- "Fail"
...
knitr::kable(QCSummary, caption='Number of samples that fail each QC criterion (excludes IPC, SC, NC & bridge samples)', 
             align='r', 
...
```{r, results='asis', eval=(params$outDetectability), out.width='100%', fig.asp=2*ceiling(length(params$xmlFiles)/2)}
# Target Boxplot
...
```{r, out.width='100%', eval=(params$outIntraPlateNorm), fig.asp=(0.2*length(runs) + 0.15), eval=indicatorSC}
...
```{r, out.width='100%', eval=(params$outIntraPlateNorm), fig.asp=(0.2*length(runs) + 0.15)}
...
```{r, out.width='100%', eval=(params$SampleBoxplot), fig.asp=1, results='asis'}
