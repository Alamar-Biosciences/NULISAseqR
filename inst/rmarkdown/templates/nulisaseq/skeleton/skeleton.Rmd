...
for(i in 1:length(xmlFiles)){
  well_order <- wellorder(runs[[i]]$samples, rowAnnotName, colAnnotName)
  par(mfrow=c(ceiling((length(ICs)+1)/2),2), oma=c(1,1,1,1), mar=c(2,2,2,0))
  ...
  for (j in 1:length(ICs)){
    plateHeatmap(runs[[i]]$Data[ICs[j],], title=paste0("Plate ", as.character(i),": ",  ICs[j]), cex=0.5, digits=(if(heatMapRel) 3 else 0), well_order=well_order,relative=heatMapRel, cex.axis=0.5)
  }
  plateHeatmap(log2(colSums(runs[[i]]$Data, na.rm=TRUE)), title=paste0('Plate ', as.character(i), ": ", 'log2(total counts)'), cex=0.5, digits=3,
               well_order=well_order, relative=heatMapRel, cex.axis=0.5)
}
...
par(mfrow=c(1,2))
boxplot(intraCV_unnorm, las=1, xlab='intra-plate CV (%)',
        ylim=c(0, max(c(intraCV_unnorm, intraCV_mCherry), na.rm=TRUE)),
        main='un-normalized', horizontal=T,
        col=alamarColorPalette(6))
abline(v=c(0:8*5), col='grey', lty=3)
boxplot(intraCV_mCherry, las=1, xlab='intra-plate CV (%)',
        ylim=c(0, max(c(intraCV_unnorm, intraCV_mCherry), na.rm=TRUE)),
        main='mCherry-normalized', horizontal=T,
        col=alamarColorPalette(6))
abline(v=c(0:8*5), col='grey', lty=3)
```
...
pandoc.header("Inter-plate %CV", 4)
...
boxplot(interCV_results, las=1, xlab='inter-plate CV (%)',
        col=alamarColorPalette(5), horizontal=T)
...
pandoc.header("Sample / Plate PCA", 2)
doPCA <- function(data, plateVar, normed=F){
  pca <- if(normed) prcomp(data, scale=F, center=F) else prcomp(log2(data+0.01), scale=F, center=F)
  title <- if(normed) "Normalized" else "Unnormalized"
  par(mar=c(4,4,2,5))
  plot(pca$x[,1], pca$x[,2], 
       col=1:length(levels(factor(plateVar))),
       pch=1:length(levels(factor(plateVar))),
...
par(mfrow=c(1,2))
e<-doPCA(dataPCAunnormal, plateVar, normed=F)
e<-doPCA(dataPCAnormed, plateVar, normed=T)
```
