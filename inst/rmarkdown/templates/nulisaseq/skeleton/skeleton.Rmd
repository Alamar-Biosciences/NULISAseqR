...
for(i in 1:length(params$xmlFiles)){
  ...
  Bridges <- NULL
  if(indicatorSC){
    SCs  <- runs[[i]]$samples$sampleName[which(grepl(paste(params$SC, collapse="|"),  runs[[i]]$samples$sampleName))]
  }
  if(indicatorBridge){
    Bridges  <- runs[[i]]$samples$sampleName[which(grepl(paste(params$Bridge, collapse="|"),  runs[[i]]$samples$sampleName))]
  }
  ...
  runSummaries[[i]] <- plateSummary(runs[[i]], ICs=ICs, IPCs=IPCs, NCs=NCs, SCs=SCs, Bridges=Bridges)
...
for (i in 1:length(params$xmlFiles)){
  ...
  if(indicatorSC){
    SC_wells[[i]]  <- runs[[i]]$samples$sampleName[which(grepl(paste(params$SC, collapse="|"), runs[[i]]$samples$sampleName))]
  }
  if(indicatorBridge){
...
for(i in 1:length(params$xmlFiles)){
  ...
  well_types <- list(IPC_wells[[i]], NC_wells[[i]])#, SC_wells[[i]], Bridge_wells[[i]])
...
for (j in 1:length(criteria)){
  for (i in 1:length(runs)){
    ...
    indFail <- which(qcSample[inds,]$status == "T")
    QCSummary[i, j+1] <- length(indFail)
  }
}
...
for( i in 1:length(runs)){
  ...
  for(j in 1:length(names(criteria))){
    ...
    indFail <- which(qcSample[inds,]$status != "T")
    indPass <- which(qcSample[inds,]$status != "F")
    xvalFail <- xval
    xvalFail[indFail] <- NA
    xvalPass <-xval
    xvalPass[indPass] <- NA
...
PCAplot <- function(data, plateID, scale=TRUE, center=TRUE, title=NULL){
  ...
  if(indicatorBridge){
    Bridge_samples <- grep(paste(params$SC, collapse="|"), colnames(data))
