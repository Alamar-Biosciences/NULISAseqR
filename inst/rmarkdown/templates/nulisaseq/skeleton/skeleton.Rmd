...
for(i in 1:length(xmlFiles)){
  ...
  boxplot(log2(mCherry_IPC$interNormData[[1]][,ordering]+0.01),
          las=1, xaxt='n',
          ylab='',
          main=paste("Plate", as.character(i), "mCherry + IPC"),
          outcex=0.5,
          col=plate_sample_colors)
  axis(1, at=1:96, labels=colnames(mCherry_IPC$interNormData[[1]])[ordering], cex.axis=0.4, las=3)
  lines(log2(mCherry_IPC$interNormData[[1]]['mCherry',ordering]+0.01), col='red')
  mtext('log2(count + 0.01)', side=2, line=2)
  legend('topleft', 'mCherry', col='red', lty=1,
         cex=0.4, bty='n')
}
