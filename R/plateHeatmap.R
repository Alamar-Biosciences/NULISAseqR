...
plateHeatmap <- function(target_data, 
  ...
  if (relative){
    med <- median(target_data, na.rm=TRUE)
    target_data <- (target_data - med)/med*100
  }
  data_matrix <- t(matrix(target_data, nrow=8, ncol=12, byrow=TRUE)[8:1,])
  if(relative){
    paletteLength <- 50
    col <- colorRampPalette(c("royalblue1", "white", "red"))(paletteLength)
    data_min <- min(data_matrix, na.rm=TRUE)
    data_max <- max(data_matrix, na.rm=TRUE)
    myBreaks <- c(seq(data_min, 0, length.out=ceiling(paletteLength/2) + 1),
              seq(data_max/paletteLength, data_max, length.out=floor(paletteLength/2)))
  ...
  mtext(title, side=3, line=1, font=2, cex.axis=1, cex=cex)
  if (print_counts==TRUE){
    rounded_matrix <- round(data_matrix, digits=digits)
    x_coords <- rep((0:(nrow(data_matrix)-1)) / (nrow(data_matrix) - 1), times=ncol(data_matrix))
    y_coords <- rep((0:(ncol(data_matrix)-1)) / (ncol(data_matrix) - 1), each=nrow(data_matrix))
    values <- as.vector(rounded_matrix)

    text(x_coords, y_coords, values, cex=cex)
  }
}
