...
```{r, include=FALSE}
QC_mCherry_table <- list()
# QC tables
for(i in 1:length(params$xmlFiles)){
  # get mCherry data
  run_i_mCherry <- runs[[i]]$Data['mCherry',]
  # remove IPCs, NCs, and SCs
  run_i_mCherry <- run_i_mCherry[!grepl(paste(params$IPC, collapse="|"), names(run_i_mCherry))]
  run_i_mCherry <- run_i_mCherry[!grepl(paste(params$NC, collapse="|"), names(run_i_mCherry))]
  run_i_mCherry <- run_i_mCherry[!grepl(paste(params$SC, collapse="|"), names(run_i_mCherry))]
  # count # of samples with mCherry outside +/- 30% of median
  mCherry_median <- median(run_i_mCherry)
  mCherry_median_plus30 <- mCherry_median + mCherry_median*(30/100)
  mCherry_median_minus30 <- mCherry_median - mCherry_median*(30/100)
  samples_above_threshold <- names(run_i_mCherry)[run_i_mCherry > mCherry_median_plus30]
  samples_below_threshold <- names(run_i_mCherry)[run_i_mCherry < mCherry_median_minus30]
  # output
  output <- data.frame(Plate=paste0('Plate ', i),
                       TotalSamples=length(run_i_mCherry),
                       NumberWarnings=length(c(samples_above_threshold,
                                               samples_below_threshold)),
                       SamplesAbove=paste(samples_above_threshold, collapse=', '),
                       SamplesBelow=paste(samples_below_threshold, collapse=', '))
  QC_mCherry_table[[i]] <- output
}

QC_mCherry_table <- do.call(rbind, QC_mCherry_table)
QC_mCherry_totals <- c('Totals', 
                       sum(as.numeric(QC_mCherry_table[,2])),
                       sum(as.numeric(QC_mCherry_table[,3])),
                       '',
                       '')
QC_mCherry_table <- rbind(QC_mCherry_table,
                          QC_mCherry_totals )

rownames(QC_mCherry_table) <- NULL
colnames(QC_mCherry_table) <- c('Plate', 'Total Samples', 'Number QC Warnings',
                                'Samples mCherry > 30% median',
                                'Samples mCherry < 30% median')
```
...
```{r, results='asis'}
# QC table
knitr::kable(QC_mCherry_table, caption='Sample QC mCherry table', 
             align='r', 
             table.attr="id=\"QC-mCherry-table\"",
...
for( i in 1:length(runs)){
  ...
  for(j in 1:length(cats)){
    ...
    val <- if(cats[j] == "Detectability") criteria[j]*100 else criteria[j]
    rmin <- min(val, as.numeric(xval), na.rm=T)
    rmax <- max(val, as.numeric(xval), na.rm=T)
    log <- if(cats[j] != "Detectability") "x" else ""
...
## Detectability
Detectability is percentage of samples above limit of detection (LOD). A target is "detectable" if at least 50% of samples are above LOD. 
...
```{r, out.width='100%', fig.asp=0.8, results='asis', eval=multipleFiles}
pandoc.header("Sample PCA", 2)

doPCA <- function(data, plateID, scale=TRUE, center=TRUE, title=NULL){
  ...
  if(!is.null(params$Bridge)) point_shapes[Bridge_samples] <- 10
  ...
  legend(par('usr')[2], par('usr')[4], 
         c(levels(plateID), 'Sample', 'IPCs', 'SCs', 'Bridge'),
...
par(mfrow=c(2,2))
doPCA(dataPCA_unnorm, plateID, scale=TRUE, center=TRUE, title="Unnormalized")
doPCA(dataPCA_mCherry, plateID, scale=TRUE, center=TRUE, title="mCherry")
doPCA(dataPCA_mCherryIPC, plateID, scale=TRUE, center=TRUE, title="mCherry + IPC")
doPCA(dataPCA_mCherryIN, plateID, scale=TRUE, center=TRUE, title="mCherry + IN")
```
