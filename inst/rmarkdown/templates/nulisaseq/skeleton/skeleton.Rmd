...
for (i in 1:length(runs)){
  ...
  for (j in 1:length(criteria$thresholds)){
    ...
    # still need to fix for NumReads and ICReads 
    # chenge delta so it makes sense on log scale
    ...
    QCSummary[i, j+1] <- sum(QC_criterion_j_data[QC_criterion_j_data$sampleName %in% runs[[i]]$SampleNames,]$status == "T")
  }
}
...
for (i in 1:length(runs)){
  ...
  for (j in 1:length(criteria$thresholds)){
    ...
    QC_fail_samples_j <- QC_criterion_j_data_samples$sampleName[QC_criterion_j_data_samples$status=='T']
...
for( i in 1:length(runs)){
  ...
  # define label colors
  color <- rep('black', length(sampleType_factor))
  color[which(sampleType_factor[well_order] == "IPC")] <- boxplot_colors[2]
  color[which(sampleType_factor[well_order] == "NC")] <- boxplot_colors[3]
  if (length(which(sampleType_factor[well_order] == "SC")) > 0 ){
    color[which(sampleType_factor[well_order] == "SC")] <- boxplot_colors[4]
  }
  if (length(which(sampleType_factor[well_order] == "Bridge")) > 0 ){
    color[which(sampleType_factor[well_order] == "Bridge")] <- boxplot_colors[5]
  }
  color <- rev(color)
  ...
  for(j in 1:length(names(criteria$thresholds))){
    ...
    if(j==1){
      ...
      axis(side=2, at=1:length(xval), 
           labels=qcSample[inds, ]$sampleName, col.axis='black', las=1,
           cex.axis=0.5)
    }
...
knitr::kable(SC_intraCV_mCherry_summary_table, caption='SC intra-plate %CV', align='r', table.attr="id=\"SC-intra-plateCV-mCherry-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)

knitr::kable(IPC_intraCV_mCherry_summary_table, caption='IPC intra-plate %CV', align='r', table.attr="id=\"IPC-intra-plateCV-mCherry-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)
```
...
pandoc.header("Intra-plate %CV table -- mCherry-normalized", 3)
knitr::kable(IPC_intraCV_mCherry_summary_table, caption='IPC intra-plate %CV', align='r', table.attr="id=\"intra-plateCV-mCherry-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)
```


### Intra-plate %CV boxplots by plate
```{r, out.width='100%', fig.asp=0.25*length(runs), eval=indicatorSC}
