...
```{r instrument-effect-test, include=FALSE, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate))}
...
```{r inst-effect-test-samples-table, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & !is.null(params$assayRunInfo)), results='asis', out.width='75%', fig.asp=1, fig.align = 'center', warning=FALSE}
if(multipleFiles & !is.null(params$assayRunInfo)){
  if(do_inst_effect_test){
    cat("\n\n---\n\n")
...
```{r inst-effect-test-volcano-plot, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & !is.null(params$assayRunInfo)), results='asis', out.width='75%', fig.asp=1, fig.align = 'center', warning=FALSE}
if(multipleFiles){
  if(do_inst_effect_test){
...
```{r save-inst-effect-test-result, eval=(params$outPlateEffect & multipleFiles & !isTRUE(plate_effect_single_plate) & params$outputPlateEffectCSV==TRUE & params$reportType=="internal" & !is.null(params$assayRunInfo)), include=FALSE}
if(multipleFiles & params$reportType != "webApp"){
  if(do_inst_effect_test){
    # make directory -- we will put the file in the outputFiles directory
    dir.create(file.path(params$outfolder, "outputFiles"), recursive = TRUE)
    ...
    # Batch effect test -- output CSV file
    write.csv(inst_effect_results[[1]]$target_table, file.path(params$outfolder, "outputFiles", "target_inst_effect_table.csv"))
  }
}
```
