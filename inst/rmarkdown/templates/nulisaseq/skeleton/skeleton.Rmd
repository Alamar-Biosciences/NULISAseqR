...
if(multipleFiles==FALSE & !is.null(params$sampleGroupCovar)){
  # get only Overall estimates
  detectability_data <- detect_output_table
  sample_types <- detectability_return_summary[[1]][2:nrow(detectability_return_summary[[1]]),1]
  sample_types <- gsub(' ', replacement = '_', sample_types)
  colnames(detectability_data) <- paste0('Target detectability (%):\n', colnames(detectability_data))
  filenames <- paste0('./figures/target_detectability_histogram_', sample_types, '.pdf')
...
if(multipleFiles==TRUE & !is.null(params$sampleGroupCovar)){
  # get only Overall estimates
  detectability_data <- detect_output_table[,grep('Overall', colnames(detect_output_table))]
  sample_types <- detectability_return_summary$Overall[2:nrow(detectability_return_summary$Overall),1]
  sample_types <- gsub(' ', replacement = '_', sample_types)
...
##########################
if(is.null(params$sampleGroupCovar) & length(runs) > 1){
  ...
  pdf('./figures/target_detectability_boxplot_all_plates.pdf', width=20, height=7)
  # make plot
  par(mar=c(4,4,2,0.5))
...
if(!is.null(params$sampleGroupCovar)){
  ...
  for(k in 1:length(sample_groups)){
    ...
    sample_group_filename <- paste0('./figures/target_detectability_boxplot_all_plates_', sample_groups[k], '.pdf')
    sample_group_title <- paste0('Target detectability: All plates, ', sample_groups[k])
