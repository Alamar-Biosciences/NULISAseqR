...
for( i in 1:length(runs)){
  ...
  for(j in 1:length(cats)){
    ...
    rmin <- if(cats[j] == "Detectability") 0.0 else min(val, as.numeric(xval), na.rm=T)
    rmax <- if(cats[j] == "Detectability") 100 else  max(val, as.numeric(xval), na.rm=T)
...
```{r, out.width='100%', fig.asp=0.4, results='asis', eval=!multipleFiles}
# PCA plot for single plate 
...
dataPCA_unnorm <- dataPCA_mCherry <- plateID <- NULL
for(i in 1:length(params$xmlFiles)){
