...
### Special Targets
IC <- c('mCherry', 'NIC_1', 'NIC_3') #Internal Control and target of interest
...
### Internal control (IC) and targets of interest
```{r, results='asis'}
if(length(ICs) == 1){
  IC_tables <- NULL
  for (i in 1:length(xmlFiles)){
...
for(i in 1:length(xmlFiles)){
  ...
  digitsTotal <- if(heatMapRel) 3 else 1
  digitsmCherry <- if(heatMapRel) 3 else 0
