...
test_that("Test that loadNULISAseq reads in an XML file and can use an alternate IC", {
  ...
  expect_true(data6$targets$targetName[which(data6$targets$hide)] == "mCherry")
  ...
  expect_true("CCL7" %in% data6$IC)
  expect_false("mCherry" %in% data6$IC)
  expect_true(all(c("mCherry", "WNT7A") %in% data6$targets$targetName[which(data6$targets$hide)]), info ="Expected both 'mCherry' and 'WNT7A' to be in hidden targets")
})
