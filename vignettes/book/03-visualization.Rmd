...
```{r basic_heatmap, fig.width=6, fig.height=8, out.width="100%", fig.align='center'}
...
```{r annotated_heatmap, fig.width=7, fig.height=8, out.width="100%", fig.align='center'}
...
- **Standardizes data** by protein (z-scores): centers and scales each protein's values
- **Performs PCA** using `PCAtools` package to identify major sources of variation
...
```{r enhanced_pca, fig.width=5, fig.height=4, out.width="100%", fig.align='center'}
pca2 <- generate_pca(
  data = data$merged$Data_NPQ,
...
```{r npq_histogram, fig.width=3.8, fig.height=3, out.width="90%", fig.align='center'}
data_long %>%
...
```{r npq_histogram_by_target, fig.width=11, fig.height=10, out.width="100%", fig.align='center'}
data_long %>%
  filter(SampleName %in% sample_list) %>%
  ggplot(aes(x = NPQ)) +
  geom_histogram(bins = 50, fill = "steelblue", alpha = 0.7) +
...
```{r npq_density, fig.width=4, fig.height=3, out.width="90%", fig.align='center'}
data_long %>%
  filter(SampleName %in% sample_list) %>%
...
```{r protein_plot, fig.width=3, fig.height=2, out.width="90%", fig.align='center'}
...
```{r correlation_plot, fig.width=3, fig.height=2, out.width="90%", fig.align='center'}
...
- ✗ Overplotting (too many points overlapping) -- use transparent colors by setting `alpha` < 1
- ✗ Missing axis labels or legends -- be sure to add these!
...
<br>
**Continue to:** [Chapter 4: Differential Abundance Analysis](#differential-abundance-analysis)
