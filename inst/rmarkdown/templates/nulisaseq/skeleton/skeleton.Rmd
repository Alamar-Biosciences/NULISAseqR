...
params:
  ...
  xmlFiles: !r c("20230325_TAP_Qatar_plate01_no_nonmatching.xml","20230325_TAP_Qatar_plate02_no_nonmatching_XML.xml")
  ...
  ## WebApp
  reportType: "internal"
  # list of names of samples to exclude
...
for (i in 1:length(params$xmlFiles)){
  ...
  if (is.null(params$sampleGroupCovar)){
    ...
    sample_identity <- runs[[i]][[params$sampleGroupCovar]]
  }
  ...
  if (length(names(plate_detect)) == 2){
    for (j in names(plate_detect$sample$detectability)){
      raw_detectability <- plate_detect$sample$detectability[[j]]
      detectability <- cbind(j,
                             plate_detect$sample$sampleNumber[[j]],
                             format(round(mean(raw_detectability, na.rm=TRUE), 1), nsmall=1),
                             format(round(sd(raw_detectability, na.rm=TRUE), 1), nsmall=1),
                             formatC(median(raw_detectability, na.rm=TRUE), width=2),
                             formatC(min(raw_detectability, na.rm=TRUE), width=2),
...
sampleboxplot <- function(data, 
                          ...
                          data_axis_label='log2(count + 0.01)'){
  par(mar=c(2,6,2,0.5))
  if(horiz==TRUE){
    sample_axis_side <- 2
    data_axis_side <- 1
  ...
  boxplot(log2(data[,data_order]+0.01),
          xlim=xlimits,
          ylim=ylimits,
          las=1, 
          yaxt='n',
  ...
  lines(cbind(log2(data['mCherry', data_order]+0.01), 1:ncol(data)), col='red', las=1)
  # add data axis label
...
for(i in 1:length(params$xmlFiles)){
  pdf(file=NULL)
  corrvals <- corrplot(cor(data_mCherry_log2_scaled[[i]], use="complete.obs"), 
                       order='hclust') 
  dev.off()
...
for(i in 1:length(params$xmlFiles)){
  ...
  breaks <- c(seq(min(data_mCherry_log2_no_NC_scaled[[i]],na.rm=TRUE), 0, length.out=ceiling(paletteLength/2) + 1), 
              seq(max(data_mCherry_log2_scaled[[i]], na.rm=TRUE)/paletteLength, 
                  max(data_mCherry_log2_scaled[[i]], na.rm=TRUE), length.out=floor(paletteLength/2)))
  ...
  vals <- data_mCherry_log2_scaled2[,1:(ncol(data_mCherry_log2_scaled2) - 1)]
...
for(i in 1:length(params$xmlFiles)){
  ...
  knitr::kable(val, caption=paste0("Plate ", as.character(i), ": Plate Layout"), 
    ...
    column_spec(column=1:13, width = "1.75cm", border_left = TRUE, border_right = TRUE) %>%
    row_spec(0:8, extra_css = "border-bottom: 1px solid")  %>%
    save_kable(file=paste0('./figures/plate_layout_', i, '.pdf'))
...
for(i in 1:length(runs)){
  ...
  # make plot
  pdf(paste0('./figures/sample_QC_IC_median_', i, '.pdf'), width=12, height=4)
  ...
  legend('bottomright', legend=c('Pass', "Fail"), col=c('green', 'red'), 
...
for(i in 1:length(params$xmlFiles)){
  ...
  ordering <- rev(ordering)
  # make plots
  pdf(paste0('./figures/sample_boxplot_', i, '.pdf'), width=4, height=9)
  par(mar=c(4,6,2,0))
  sampleboxplot(mCherry_IPC$interNormData[[i]], data_order=ordering, 
...
##########################
# intra-plate and inter-plate CV density plots
##########################
# intra-plate CV 
pdf(paste0('./figures/intraCV_density.pdf'), 
    width=5, height=5)
...
plot(density(intraCV_mean, na.rm=TRUE),
     main='Intra-plate CV (%)',
     xlab='CV (%)', las=1)
x <- density(intraCV_mean, na.rm=TRUE)$x
x <- c(x[1], x, x[length(x)])
y <- c(0, density(intraCV_mean, na.rm=TRUE)$y, 0)
polygon(x, y,
        col=colors[[1]][5],
        border=colors[[1]][2])
abline(v=mean(intraCV_mean, na.rm=TRUE), col=colors[[1]][2], lwd=2)
abline(v=median(intraCV_mean, na.rm=TRUE), col=colors[[1]][2], lty=2, lwd=2)
legend('topright', c(paste0('mean CV% = ', format(round(mean(intraCV_mean, na.rm=TRUE), 1), nsmall=1)), 
       ...
       lty=c(1,2), col=colors[[1]][2], bty='n', lwd=2, cex=1.25)
dev.off()

# inter-plate CV
if(multipleFiles==TRUE){
  pdf(paste0('./figures/interCV_density.pdf'), 
      width=5, height=5)
  colors <- alamarColorPalette(2, nReps = 5, palette = 2)
  par(mar=c(4.5, 4.5, 2, 1))
  if(indicatorSC==TRUE){
  ...
  plot(density(interCV, na.rm=TRUE),
       main='Inter-plate CV (%)',
       xlab='CV (%)', las=1)
  x <- density(interCV, na.rm=TRUE)$x
  x <- c(x[1], x, x[length(x)])
  y <- c(0, density(interCV, na.rm=TRUE)$y, 0)
  polygon(x, y,
          col=colors[[1]][5],
          border=colors[[1]][2])
  abline(v=mean(interCV, na.rm=TRUE), col=colors[[1]][2], lwd=2)
  abline(v=median(interCV, na.rm=TRUE), col=colors[[1]][2], lty=2, lwd=2)
...
##########################
# intra-plate CV 
pdf(paste0('./figures/intraCV_histogram.pdf'), 
    width=5, height=5)
colors <- alamarColorPalette(2, nReps = 5, palette = 2)
par(mar=c(4.5, 4.5, 2, 1))
if(indicatorSC==TRUE){
...
# inter-plate CV
if(multipleFiles==TRUE){
  pdf(paste0('./figures/interCV_histogram.pdf'), 
      width=5, height=5)
  colors <- alamarColorPalette(2, nReps = 5, palette = 2)
  par(mar=c(4.5, 4.5, 2, 1))
  if(indicatorSC==TRUE){
...
##########################
# target detectability density plot
##########################
if(multipleFiles==TRUE){
  detectability_data <- as.numeric(detectability_return_targets$Overall[,'all'])
} else {
  detectability_data <- as.numeric(detectability_return_targets$`Plate  1`[,1])
}

pdf('./figures/target_detectability_density.pdf',
    width=5, height=5)
par(mar=c(4.5, 4.5, 2, 1))
plot(density(detectability_data, na.rm=TRUE),
     main='Target detectability (%)',
     xlab='Detectability (%)', las=1)
x <- density(detectability_data, na.rm=TRUE)$x
x <- c(x[1], x, x[length(x)])
y <- c(0, density(detectability_data, na.rm=TRUE)$y, 0)
polygon(x, y,
        col=colors[[1]][5],
        border=colors[[1]][2])
abline(v=50, col=colors[[2]][2], lwd=2)
text(50, 0.013, '50%', pos=4, col=colors[[2]][2])
...
##########################
# target detectability histogram
##########################

pdf('./figures/target_detectability_histogram.pdf',
    width=5, height=5)
par(mar=c(4.5, 4.5, 2, 1))
...
##########################
for(i in 1:length(params$xmlFiles)){
  pdf(paste0('./figures/target_detectability_boxplot_', i, '.pdf'), width=20, height=7)
  targetBoxplot(intraPlateNormData[[i]][[1]]$normData,
                title=PlateNames[i],
                subtractLOD = TRUE,
                blanks = runs[[i]]$NC,
