...
IPCs are pooled plasma controls used for normalization between plates.
 <img src="figures/QC report/Read Summary IPC.png" style="width:100%; max-width:800px;">

#### Sample Controls (SC)
SCs are pooled plasma controls sourced independently from IPCs, and are used to assess intra-plate and inter-plate coefficient of variation (CV). 
<img src="figures/QC report/Read Summary SC.png" style="width:100%; max-width:800px;">
 
#### Negative Controls (NC)
NC wells contain assay buffer. For each plate, NCs are used to determine Limit of Detection (LOD) and give a measure of assay background for each target.
<img src="figures/QC report/Read Summary NC.png" style="width:100%; max-width:800px;">
 
### Quality Control Metrics {-}

The QC report evaluates quality at three levels:
...
Run-level QC metrics assess the overall quality of each assay plate/run. These are evaluated on both unnormalized (raw) and IPC-normalized data.


<img src="figures/QC report/Plate QC.png" style="width:100%; max-width:800px;">

Run-level QC metrics assess the overall quality of each assay run. These are evaluated on both unnormalized (raw) and IPC-normalized data.
...
- **Definition**: CV of total sum of Parseable Matching reads across all IPC samples
...
##### 3. IPC Target CV {-}
- **Definition**: Median intra-plate CV of IC-IPC normalized reads for all IPC targets across replicates
...
- **Interpretation**:
  ...
  - ✗ Median CV > `r paste0(NULISAseqR:::MAX_MEDIAN_IPC_TARGET_CV * 100, "%")`: Some targets showing poor precision
...
#### Sample QC Criteria {- #sample-qc-criteria}
...
- **Definition**: Minimum number of reads required per sample
...
#### Sample QC Plots {- #sample-qc-plots}
...
- **Sample Correlation Plots**: Shows clustered heatmap of sample correlation matrix. Samples with high correlation will cluster together into blocks on the diagonal, which can be helpful for identifying sample replicates or multiple samples from the same individuals at different time points.
- **Sample and Target Clustering**: Shows clustered heatmap of scaled NPQ matrix by sample and target
- **Sample PCA**: Helpful for identifying outlier samples and batch effects
...
```{r sample-qc-plot-plate1, echo=FALSE, fig.cap="Sample QC plots for Plate_01", out.width="100%", out.extra='style="max-width:800px;"'}
knitr::include_graphics("figures/QC report/Sample QC plots - Plate01.png")
```

```{r sample-qc-plot-plate2, echo=FALSE, fig.cap="Sample QC plots for Plate_02", out.width="100%", out.extra='style="max-width:800px;"'}
knitr::include_graphics("figures/QC report/Sample QC plots - Plate02.png")
```

**Sample Boxplots Before and After Normalization** show the distribution of protein expression across samples before and after normalization with internal controls:

```{r sample-boxplot-plate1, echo=FALSE, fig.cap="Sample boxplots for Plate_01 after normalization", out.width="100%", out.extra='style="max-width:800px;"'}
knitr::include_graphics("figures/QC report/Sample boxplot - Plate01.png")
```

```{r sample-boxplot-plate2, echo=FALSE, fig.cap="Sample boxplots for Plate_02 after normalization", out.width="100%", out.extra='style="max-width:800px;"'}
knitr::include_graphics("figures/QC report/Sample boxplot - Plate02.png")
```

**Interpreting Sample Boxplots**: After IC normalization, sample boxplots should show similar median values and distributions. Outlier samples with shifted or compressed distributions may indicate quality issues. For example, in Plate_01, sample SMI_G05_Donor65 shows a high IC value before normalization (left), and after normalization this sample shows a lower distribution relative to other samples (right), indicating that IC normalization was too aggressive (Figure \@ref(fig:sample-boxplot-plate1)). 
...
- **Usage**: 
  ...
  - Consider filtering targets with high proportion of values below LOD for differential analysis 
  - Visualized in target detectability boxplots where NPQ levels are shown relative to LOD

**Special target categories**: There are two special target categories to be aware of which are treated differently regarding LOD, detectability, and QC metric reporting:

**High abundance targets:**

Examples of high abundance targets:

- **Inflammation Panel**: **CRP** and **KNG1** 
- **CNS Panel**: **APOE** and **CRP** 
- **Neuro 220 Panel**: **APOA1**, **APOA2**, **APOE**, **APOH**, **C1q**, **CRP**, **PPBP**, **PROS1**, and **SERPINA3** 
- **Mouse Panel**: **Crp** 

These targets have high endogenous levels in plasma and serum samples. To accommodate them in the same panel as lower-abundance targets, special tuning strategies and algorithms are applied. As a result, LOD and detectability are not reported and these targets are excluded from sample QC metrics and detectability summary statistics. For sample types beyond validated matrices (plasma and serum), the algorithm may not be applicable and NPQ is not reported. 

**Rare case targets:**

Examples of rare case targets:

- **Neuro 220 Panel**: **APOE4**, **mHTT-exon1**, **pLRRK2-S1292**, **pPRKN-S65**, **pQ-ATXN3**, **pQ-HTT**, **pRAB10-T73**, **pRAB12-S106**, and **pRAB29-T71**  

These targets include rare isoforms, mutations, and phosphorylation sites present only in a small fraction of the population. Unlike high-abundance targets, they do have a meaningful LOD. Individual detectability values are computed and shown in target detectability tables and boxplots. Low detectability reflects true biology (low abundance of the target in the measured samples) and does not indicate assay failure for these targets. However, these targets are excluded from sample QC metrics and detectability summary statistics (mean, SD, median, min, max, number of detectable targets) to avoid skewing panel-level aggregate metrics.

### Coefficient of Variation (CV)

CV analysis assesses assay precision within and between plates:
...
The table reports batch effect targets with the following metrics:

- run ICC > 10%
- significant F-test (unadjusted p < 0.01)
- significant pairwise test for at least one run (Tukey-adjusted p < 0.01)
...
- Targets with high batch effect ICC should be interpreted cautiously in downstream analyses
...
It is best practice to always randomize samples to plates so that technical run-related variation can be distinguished from biological variation. Please refer to **Tech Note - NULISA<sup>TM</sup> multiplex normalization & quality control** for advice on sample randomization. See [Technical Documentation](#technical-documentation) in the Additional Resources chapter. 
...
Certain high abundance targets use specialized normalization algorithms optimized for specific sample matrix types. NPQ values for these targets will not be reported for non-validated sample matrices. For example, in the following panels, NPQ values for the listed targets are only reported in plasma and serum samples:
...
These proteins have high endogenous levels in plasma and serum that require matrix-specific tuning. The normalization algorithm may not be applicable to other sample matrix types (e.g., tissue lysates, saliva, urine), and NPQ values will not be generated for these specific targets in sample matrix types other than plasma and serum. Note that LOD and detectability are not reported for these targets — see [Target QC](#target-qc) above for more details.
...
- **Non-normal distributions**: In typical sample types, targets with < 50% detectability often have bimodal or highly skewed distributions due to a mixture of detected values and below-LOD measurements
- **Heteroscedasticity**: Low-detectability targets may violate homoscedasticity assumptions for statistical methods that assume equal variance between groups
- **Statistical power and false positives**: Proteins detected in only a minority of samples provide limited power for detecting biological differences, or they could generate false positive findings if distributional assumptions are violated
...
- **> 50% detectability**: Minimum threshold for robust statistical analysis
  - Ensures data has approximate normal distribution after log-transformation
  - Provides adequate power for differential abundance analysis
...
In studies with diverse sample types or conditions, consider calculating detectability within relevant subgroups. The easiest way to do this is to use the `sample_group_covar` parameter of the `importNULISAseq` function (see [Automated Subgroup Detectability Calculation with XML](#auto-subgroup-detect) below). However, this assumes that the subgroup variable is already embedded in the xml files. If that is not the case, you may use code such as the following:
...
##### Automated Subgroup Detectability Calculation with XML {#auto-subgroup-detect}
...
- **Discuss batch effects**: Randomize samples to plates to enable detection of batch effects; coordinate on plate layout and run order to minimize batch effects
...
- **Seek expert input**: Consult with Alamar team if needed
