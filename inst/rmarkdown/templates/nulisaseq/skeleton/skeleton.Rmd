...
```{r, include=FALSE}
# calculate detectability for each plate / target
...
for(i in 1:length(params$xmlFiles)){
  ...
  data_mCherry_log2_scaled[[i]] <- t(scale(t(log2(intraPlateNormData[[i]]$mCherry$normData+0.01)),
...
for(i in 1:length(params$xmlFiles)){
  ...
  well_types <- list(Bridge_wells[[i]], IPC_wells[[i]], NC_wells[[i]])
...
kable_styling(knitr::kable(detectability_targets, caption='Target Detectability %', align='r', table.attr="id=\"detectability-indepth\"",
                           format.args=list(big.mark = ",", digits=2)), bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) 
```
...
for(i in 1:length(params$xmlFiles)){
  well_types <- list(Bridge_wells[[i]], IPC_wells[[i]], NC_wells[[i]])
  mycolors <- wellcolors(data_mCherry_log2_scaled[[i]], well_types, boxplot_colors)
  myBreaks <- c(seq(min(data_mCherry_log2_scaled[[i]]), 0, length.out=ceiling(paletteLength/2) + 1), 
                seq(max(data_mCherry_log2_scaled[[i]])/paletteLength, max(data_mCherry_log2_scaled[[i]]), length.out=floor(paletteLength/2)))
  data_mCherry_log2_scaled2 <- data.frame(t(data_mCherry_log2_scaled[[i]]))
  data_mCherry_log2_scaled2$status <- rep(1, nrow(runs[[i]]$samples))
  data_mCherry_log2_scaled2$colors <- mycolors
  e <- pheatmap(data_mCherry_log2_scaled2[,1:(length(data_mCherry_log2_scaled2[1, ]) - 2)],
                main = paste("Plate", i), fontsize=5,
                color=myColor,
                breaks=myBreaks,
                show_colnames=FALSE, silent=TRUE)
  cols <- data_mCherry_log2_scaled2[order(match(rownames(data_mCherry_log2_scaled2), e$gtable$grobs[[5]]$label)), ]$colors
