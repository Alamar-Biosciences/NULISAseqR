...
for (i in 1:length(params$xmlFiles)){
  normData[[i]] <- intraPlateNormData[[i]]$mCherry$normData
...
for(i in 1:length(params$xmlFiles)){
  data_mCherry_scaled[[i]] <- scale(log2(intraPlateNormData[[i]]$mCherry$normData+0.01))
...
if(length(ICs) == 1){
  ...
  for(i in 1:length(params$xmlFiles)){
    ...
    cat(knitr::kable(runSummaries[[i]]$IC_table, caption=paste0("Plate ", as.character(i), ": IC summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
  }
}
```
...
for(i in 1:length(params$xmlFiles)){
  ...
  cat(knitr::kable(runSummaries[[i]]$NC_table, caption=paste0("Plate ", as.character(i), ": NC summaray"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
