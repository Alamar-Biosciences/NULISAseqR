---
title: "Study Name <br>NULISAseq 200-plex Inflammation Panel"
output:
  html_document:
    toc: yes
date: "`r Sys.Date()`"
...
###############################################################################
#IC <- "InternalControl"
#IPC <- "InterPlateControl"
#NC <- "NegativeControl"
IC <- c('mCherry', 'NIC_1', 'NIC_3')
IPC <- c('IPC')
NC <- c('NC')
Bridge <- NULL#c('Donor')
heatMapRel <- TRUE
...
xmlFiles <- paste0(dataDir, "/", xmlFiles)
...
for(i in 1:length(xmlFiles)){
  ...
  runs[[i]] <- readNULISAseq(xmlFiles[i], plateID=plateID)
  ...
  RunNames <- c(RunNames, paste("Run", i, sep=" "))
}
names(runs) <- xmlFilePlateNames
names(runSummarys) <- xmlFilePlateNames
colnames(read_summary) <- RunNames
...
for (i in 1:length(xmlFiles)){
  ...
  IPCs <- runs[[i]]$samples$sampleName[which(grepl(paste(IPC, collapse="|"), runs[[i]]$samples$sampleName))]
  Bridges <-runs[[i]]$samples$sampleName[which(grepl(paste(Bridge, collapse="|"), runs[[i]]$samples$sampleName))]
  plateID <- if(xmlFilePlateNames[i] == "") paste0("run", i) else xmlFilePlateNames[i]
  IPC_wells[[i]] <- paste0(plateID, "_", IPCs)
  Bridge_wells[[i]] <-paste0(plateID,"_", Bridges)
  NCs  <- runs[[i]]$samples$sampleName[which(grepl(paste(NC, collapse="|"),  runs[[i]]$samples$sampleName))]
  NCs  <- paste0(plateID, "_", NCs) 
  NC_wells[[i]] <- paste0(plateID, "_", NCs)
} 
...
colnames(intraCV_unnorm) <- RunNames

# calculate intra-plate CVs mCherry
colnames(intraCV_mCherry) <- RunNames
...
rownames(intraCV_unnorm_summary_table) <- RunNames
...
rownames(intraCV_mCherry_summary_table) <- RunNames
...
* **Goal:** Determine effect of various CAR-T treatments on the inflammatory markers in the 
NULISAseq 200-plex inflammation panel.
* Sample description:
...
for(i in 1:length(xmlFiles)){
  ...
  vals <- runs[[i]]$samples[well_order,]
  vals$sampleBarcode <- NULL
  vals$`non-matching` <- NULL
  vals$matching <- NULL
  rownames(vals)<-NULL
  if (!is.null(plateAnnotName)){
    cmd <- paste0("vals$", plateAnnotName, "<-NULL")
    eval(parse(text=cmd))
  }
  print(knitr::kable(row.names=NA, vals, caption=paste("Run", as.character(i), "Plate/Sample Layout"), align='r', format.args=list(big.mark = ",")))
  cat("\n")
}
```
...
## Reads summary
```{r, results='asis'}
knitr::kable(read_summary, caption='', align='r', format.args=list(big.mark = ","))
```

---

## Internal control (IC) summary
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  print(knitr::kable(runSummarys[[i]]$IC_table, caption=paste("Run", as.character(i), "IC summary"), align='r', format.args=list(big.mark = ",")))
  cat("\n")
}
```
...
## Inter-plate control (IPC) summary
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  print(knitr::kable(runSummarys[[i]]$IPC_table, caption=paste("Run", as.character(i), "IPC summary"), align='r', format.args=list(big.mark = ",")))
  cat("\n")
}
```
---

## Negative control (NC) summary
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  print(knitr::kable(runSummarys[[i]]$NC_table, caption=paste("Run", as.character(i), "NC summaray"), align='r', format.args=list(big.mark = ",")))
  cat("\n")
}
```
...
for(i in 1:length(xmlFiles)){
  well_order <- wellorder(runs[[i]]$samples, rowAnnotName, colAnnotName)
  par(mfrow=c(ceiling((length(ICs)+1)/2),2), mar=c(4,2,2,0))
  for (j in 1:length(ICs)){
    plateHeatmap(runs[[i]]$Data[ICs[j],], title=paste("Plate", as.character(i), ICs[j]), cex=0.5, digits=(if(heatMapRel) 3 else 0), well_order=well_order,relative=heatMapRel)
  }
  plateHeatmap(log2(colSums(runs[[i]]$Data, na.rm=TRUE)), title=paste('Plate', as.character(i), 'log2(total counts)'), cex=0.5, digits=3,
...
for(i in 1:length(xmlFiles)){
  ...
  plate_sample_type[grepl(paste0(Bridges, collapse="|"), colnames(runs[[i]]$Data))] <- '2_donor'
  plate_sample_type[grepl(paste0(IPCs, collapse="|"), colnames(runs[[i]]$Data))] <- '3_IPC'
  plate_sample_type[grepl(paste0(NCs, collapse="|"), colnames(runs[[i]]$Data))] <- '4_NC'

  boxplot_colors <- unlist(lapply(alamarColorPalette(n=4, tint='light', nReps=4), function(x) x[3]))
  plate_sample_colors <- boxplot_colors[as.numeric(as.factor(plate_sample_type))[order(plate_sample_type)]]

  par(mfrow=c(2,1), mar=c(6,3,2,0))
  boxplot(log2(runs[[i]]$Data[,order(plate_sample_type)]+0.01),
          las=1, xaxt='n',
          ylab='',
          main=paste("Plate", as.character(i), "unnormalized"),
          outcex=0.5,
          col=plate_sample_colors)
  axis(1, at=1:96, labels=colnames(runs[[i]]$Data)[order(plate_sample_type)], cex.axis=0.4, las=3)
  lines(log2(runs[[i]]$Data['mCherry',order(plate_sample_type)]+0.01), col='red')
  mtext('log2(count + 0.01)', side=2, line=2)
  legend('topleft', 'mCherry', col='red', lty=1,
         cex=0.4, bty='n')

  boxplot(log2(mCherry_IPC$interNormData[[1]][,order(plate_sample_type)]+0.01),
          las=1, xaxt='n',
          ylab='',
          main=paste("Plate", as.character(i), "mCherry + IPC"),
          outcex=0.5,
          col=plate_sample_colors)
  axis(1, at=1:96, labels=colnames(mCherry_IPC$interNormData[[1]])[order(plate_sample_type)], cex.axis=0.4, las=3)
  lines(log2(mCherry_IPC$interNormData[[1]]['mCherry',order(plate_sample_type)]+0.01), col='red')
  mtext('log2(count + 0.01)', side=2, line=2)
  legend('topleft', 'mCherry', col='red', lty=1,
         cex=0.4, bty='n')
}
