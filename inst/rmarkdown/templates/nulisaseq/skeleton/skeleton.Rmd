...
params:
  ...
  NC: !r c("NC")
  Bridge: !r NULL
  dataDir: !r '/Users/dkuo/Projects/NULISAseqR/data'
  rowAnnotName: !r "Annot2"
  colAnnotName: !r "Annot3"
  plateAnnotName: !r "Annot1"
  heatMapRel: !r TRUE
  study_name: !r "Study Name"
  assayName: !r "NULISAseq 200-plex Inflammation Panel"
  sample_desc: !r "This is a sample description"
  goal_desc: !r "NULISAseq 200-plex inflammation panel"
...
matrixify <- function(runSamples, rowAnnotName, colAnnotName, plate=F){
...
runSummarys <- vector('list', length(params$xmlFiles))
read_summary <- NULL
PlateNames <- NULL
...
rownames(read_summary)[length(params$xmlFiles)] <- 'Total samples * targets'
read_summary <- t(read_summary)
```

```{r, include=FALSE}
run_lod <- vector('list',length(params$xmlFiles))
for (i in 1:length(params$xmlFiles)){
  # replace NAs with 0
...
plateNorms <- vector("list", length(params$xmlFiles))
for (i in 1:length(params$xmlFiles)){
...
for (i in 1:length(params$xmlFiles)){
  ...
  IPC_wells_orig[[i]] <- runs[[i]]$samples$sampleName[which(grepl(paste(params$IPC, collapse="|"), runs[[i]]$samples$sampleName))]
  if(!is.null(params$Bridge)){
    Bridge_wells_orig[[i]] <- runs[[i]]$samples$sampleName[which(grepl(paste(params$Bridge, collapse="|"), runs[[i]]$samples$sampleName))]
    if(xmlFilePlateNames[i] != ""){
      Bridge_wells[[i]] <- if(xmlFilePlateNames[i] != "") paste0(xmlFilePlateNames[i],"_", Bridge_wells_orig[[i]]) else Bridge_wells_orig[[i]]
    }
  }
  NC_wells_orig[[i]]  <- runs[[i]]$samples$sampleName[which(grepl(paste(params$NC, collapse="|"), runs[[i]]$samples$sampleName))]
  if(xmlFilePlateNames[i] != ""){
    IPC_wells[[i]] <- paste0(xmlFilePlateNames[i], "_", IPC_wells_orig[[i]])
    NC_wells[[i]]  <- paste0(xmlFilePlateNames[i], "_", NC_wells_orig[[i]]) 
  }else{
    IPC_wells[[i]] <- IPC_wells_orig[[i]]
    NC_wells[[i]]  <- NC_wells_orig[[i]] 
  }
} 
...
# run-run plates 1-6
# un-normalized data
scs <- vector("list", length(params$xmlFiles))
...
for (i in 1:length(params$xmlFiles)){
  ...
  detect_samples <- detect_samples[-which(grepl(paste(params$IPC, collapse="|"), detect_samples))]
  detect_samples <- detect_samples[-which(grepl(paste(params$NC, collapse="|"), detect_samples))]
  if(!is.null(params$Bridge)){
    detect_samples <- detect_samples[-which(grepl(paste(params$Bridge, collapse="|"), detect_samples))]
  }
...
```{r, results='asis'}
for(i in 1:length(params$xmlFiles)){
  val <- matrixify(runs[[i]]$samples, params$rowAnnotName, params$colAnnotName, plate=T)
  B_well_orig <- if (!is.null(params$Bridge)) Bridge_wells_orig[[i]] else NULL
  well_types <- list(B_well_orig, IPC_wells_orig[[i]], NC_wells_orig[[i]])
  for (j in 1:length(well_types)){
    if (j == 1 && is.null(params$Bridge)){
      next
    }
  ...
  cat(knitr::kable(val, caption=paste0("Plate ", as.character(i), ": Plate/Sample Layout"), align="c", escape=F, table.attr=table.attr) %>%
      kable_styling(font_size=9, bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) %>%
      column_spec(column=1:12, width = "1.75cm", border_left = TRUE, border_right = TRUE) %>%
...
## Read summary
```{r, results='asis'}
knitr::kable(read_summary, caption='', align='r', format.args=list(big.mark = ","), table.attr="id=\"read-summary\"") %>% 
...
if(length(ICs) == 1){
  ...
  cat(knitr::kable(IC_tables, caption="mCherry (IC) summary", align='r', format.args=list(big.mark = ","), table.attr="id=\"IC-summary\"") %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))  
}else{
  for(i in 1:length(params$xmlFiles)){
    table.attr <- paste0("id=\"IC-summary", i, "\"")
    cat(knitr::kable(runSummarys[[i]]$IC_table, caption=paste0("Plate ", as.character(i), ": IC summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
  }
}
```
...
for(i in 1:length(params$xmlFiles)){
  ...
  cat(knitr::kable(runSummarys[[i]]$IPC_table, caption=paste0("Plate ", as.character(i), ": IPC summary"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
---
...
for(i in 1:length(params$xmlFiles)){
  ...
  cat(knitr::kable(runSummarys[[i]]$NC_table, caption=paste0("Plate ", as.character(i), ": NC summaray"), align='r', format.args=list(big.mark = ","), table.attr=table.attr) %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE))
}
```
...
for(i in 1:length(params$xmlFiles)){
  ...
  logVals <- log2(colSums(runs[[i]]$Data, na.rm=T))
  well_order <- wellorder(runs[[i]]$sample, params$rowAnnotName, params$colAnnotName)
  
  val <- vals
  val[which(!is.na(vals))] <- logVals[well_order]
  plateHeatmap(as.numeric(val), title=paste0('Plate ', as.character(i), ": ", 'log2(total counts)'), cex=0.5, digits=digitsTotal,
               relative=params$heatMapRel, cex.axis=0.5)
  for (j in 1:length(ICs)){
    val <- vals
    val[ which(!is.na(vals))] <- runs[[i]]$Data[ICs[j],][well_order]
    plateHeatmap(as.numeric(val), title=paste0("Plate ", as.character(i),": ",  ICs[j]), cex=0.5, digits=digitsmCherry, relative=params$heatMapRel, cex.axis=0.5)
  }
}
```
...
### Detectability table
```{r, results='asis'}
kable_styling(knitr::kable(detectability_summary, caption='Detectability Summary', align='r', table.attr="id=\"detectability-summary\"",
             format.args=list(big.mark = ",", digits=2)), bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)
kable_styling(knitr::kable(detectability_tar, caption='Target Detectability %', align='r', , table.attr="id=\"detectability-indepth\"",
             format.args=list(big.mark = ",", digits=2)), bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE) 
```
...
boxplot(intraCV_vals
        ...
        main='Intra-plate CV (* - Normalized)', horizontal=T, cex.axis=0.5, lex.order=T,
...
for(i in 1:length(params$xmlFiles)){
  ...
  dev.off()
  B_well_orig <- if (!is.null(params$Bridge)) Bridge_wells_orig[[i]] else ""
  well_types <- list(B_well_orig, IPC_wells_orig[[i]], NC_wells_orig[[i]])
...
for(i in 1:length(params$xmlFiles)){
  ...
  data_mCherry_scaled2$status <- rep(1, nrow(runs[[i]]$samples)) # 
...
pandoc.header("Inter-plate %CV", 3)
knitr::kable(interCV_summary_table, caption='', align='r', table.attr="id=\"inter-plateCV-summary\"",
             format.args=list(big.mark = ",", digits=3)) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE)
```
...
doPCA <- function(data, plateVar, normed=F, scale=F, center=F, title="Unnormalized"){
...
for(i in 1:length(params$xmlFiles)){
  ...
  dataPCAnormed <- cbind(dataPCAnormed, removeTargetsSamples(data_mCherry_scaled[[i]], params$NC, params$IPC, params$Bridge))
  ...
  plateVar <- c(plateVar, rep(paste0("Plate",i),length(removeTargetsSamples(data_mCherry_scaled[[i]], params$NC, params$IPC, params$Bridge)[1,])))
}

par(mfrow=c(2,2))
e <- doPCA(dataPCAunnormal, plateVar, normed=F, scale=T, center=T, title="Unnormalized")
e <- doPCA(dataPCAnormed, plateVar, normed=T, scale=T, center=T, title="mCherry")
e <- doPCA(dataPCAnormedIPC, plateVar, normed=T, scale=T, center=T, title="mCherry + IPC")
e <- doPCA(dataPCAnormedIN, plateVar, normed=T, scale=T, center=T, title="mCherry + IN")
```
