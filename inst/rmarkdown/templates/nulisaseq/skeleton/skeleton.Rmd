...
for(i in 1:length(xmlFiles)){
  ...
  well_types <- list(IPC_wells_orig[[i]], NC_wells_orig[[i]], Bridge_wells_orig[[i]])
  colors <- c("coral", "cyan", "chartreuse")
  for (j in 1:length(well_types)){
    inds <- which(grepl(paste0(well_types[[j]], collapse="|"), val))
    if(length(inds) > 0){
      val[inds] <- cell_spec(val[inds], "html", background=colors[j])
    }
  }
  cat(knitr::kable(val, caption=paste("Run", as.character(i), "Plate/Sample Layout"), align="c",escape=F) %>% kable_styling(font_size=9))
}
```
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$IC_table, caption=paste("Run", as.character(i), "IC summary"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())  
}
```
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$IPC_table, caption=paste("Run", as.character(i), "IPC summary"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())
}
```
---
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$NC_table, caption=paste("Run", as.character(i), "NC summaray"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())
}
```
