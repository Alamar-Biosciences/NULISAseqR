...
params:
  ...
  highlight_TAP_report_fields: !r TRUE
...
#'
#' Convenience function that formats excluded targets and prints them using pandoc.p
#'
...
#'
#' Convenience function that formats excluded samples and prints them using pandoc.p
#'
...
pandoc.p("Each sample of the assay is spiked with the same concentration of at least one internal control (IC). This is used both as part of a well normalization procedure and as a method for assessing the uniformity of the assay run. SD: standard deviation. CV: coefficient of variation") 
IC_name <- Reduce(intersect, lapply(runs, function(x) x$IC))
...
pandoc.header("Negative controls outlier table", 4)
pandoc.p("The following values for NC wells and targets were identified as outliers and excluded from LOD calculations.")
if(!is.null(blank_outlier_table)){
  ...
  pandoc.p("No NC outliers were identified.")
}
```
...
pandoc.p(paste0("Heatmaps show the ", heatmap_description, " for log2(total counts) and the specified internal control(s)."))
for(i in 1:numFiles){
...
if(!is.null(IC_zero_samples_message)) {
  pandoc.p(IC_zero_samples_message)
}
...
pandoc.header("Target QC plots", 4)
for( i in 1:numFiles){
  # do Target QC
...
pandoc.header("Top % Reads", 3)
pandoc.header("Top 10 targets", 4)
pandoc.p('For top 10 targets by % reads on each plate, the table shows target name, % of grand total plate reads, and total target reads. Note that all targets are considered including any excluded or IC targets.')
...
pandoc.header("Top 10 samples", 4)
pandoc.p('For top 10 samples by % reads on each plate, the table shows sample name, % of grand total plate reads, and total sample reads. Note that all samples are considered including any excluded or control samples.')
...
pandoc.p(paste0("Plot shows target IC-", CAL_IPC, "-normalized count distributions relative to LOD. For each target, log2(LOD) was subtracted from log2(count) for IC-normalized data. Reverse-curve quantified targets are denoted by a \"*\" and are plotted as forward-curve targets for compatibility"))
...
if(is.na(AQ_quant[1])){
  pandoc.p('LLOQ and ULOQ are not present in run data for one or more runs. Quantifiability cannot be calculated.')
...
pandoc.header("Intra-plate CV table -- AQ", 4)
...
pandoc.p(paste0('CVs reported here were calculated on the aM units AQ data. Based on ', n_AQ_targets, ' AQ targets.'))
...
pandoc.header("Inter-plate CV table -- AQ", 4)
...
pandoc.p("Targets with any intra-plate CV > 30% are highlighted in red.")
printExcludedTargets(params$excludeTargets, PlateNames)
...
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0(AQSC_SC, " intra-plate vs inter-plate CV scatterplot"), 3)
...
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0(CAL_IPC, " intra-plate vs inter-plate CV scatterplot"), 3)
...
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0(AQSC_SC, " / ", CAL_IPC, " ratio plot"), 2)
pandoc.p(paste0("Plot shows the log2-transformed ratio (left) or the unlogged ratio (right) of the mean IC-normalized counts for ", AQSC_SC, " vs ", CAL_IPC, "."))
...
knitr::asis_output("\n\n---\n\n")
pandoc.header(paste0(AQSC_SC, " / NC ratio plot"), 2)
pandoc.p(paste0("Plot shows the log2-transformed ratio of the mean IC-normalized counts for ", AQSC_SC, " versus NC. Asterisk next to target name indicates the NC mean was zero for one or more plates, so ", AQSC_SC, " / NC ratio could not be calculated for these plates."))
...
pandoc.p("Plot shows the Pearson correlation between samples. Hierarchical clustering is done using complete linkage.")
...
if(multipleFiles){
  pandoc.p("\n\n---\n\n")
  pandoc.header("Batch effect assessment", 2)
  ...
  if(length(batch_effect_noDetectability_targets) > 0){
    pandoc.p(paste0('Note: Rare variant-specific, PTM, or other special targets were excluded from batch effect assessment: ',
  ...
  pandoc.list(plate_effect_column_descriptions)
...
if(multipleFiles){
  pandoc.p("\n\n---\n\n")
  pandoc.header("Batch effect: Target QC summary", 3)
  
  pandoc.p(paste0("A target will have a QC warning if run ICC > ", params$plate_effect_icc_threshold, "%, significant F-test (unadjusted p < ", params$plate_effect_pval_ANOVA, ") and significant pairwise tests (Tukey-adjusted p < ", params$plate_effect_pval_pairwise, ") for at least one run."))
  
  if (is.null(plate_effect_boxplot_targets)){
    pandoc.p("**There were no targets with a batch effect QC warning.**")
  } else if (!is.null(plate_effect_boxplot_targets)){
    if (length(plate_effect_boxplot_targets)==0){
      pandoc.p("**There were no targets with a batch effect QC warning.**")
    } else {
      pandoc.p(paste0("**", length(plate_effect_boxplot_targets), " targets have a batch effect QC warning.**"))
      ...
      pandoc.list(plate_effect_target_column_descriptions)
  ...
  if (is.null(plate_effect_boxplot_targets)){
    ...
    if (length(plate_effect_boxplot_targets)==0){
      ...
      if(length(plate_effect_boxplot_targets) != n_boxplots){
        pandoc.p(paste0("Plots for the top ", n_boxplots, " targets based on plate ICC are shown below."))
      }
...
if(multipleFiles){
  pandoc.header("Target means by run heatmap", 3)
  pandoc.p('Sample mean NPQ for each target is shown run in the heatmap. Target values are centered and scaled. Control samples were excluded from mean calculations.')
...
if(multipleFiles & numFiles >= 3){
  pandoc.header("PCA by run", 3)
  if(params$reportType != "webApp"){
    pandoc.p('PCA was performed on the sample mean by target and run matrix shown in the heatmap above.')
  } else {
    pandoc.p('PCA was performed on the sample mean by target and run matrix.')
  }
  par(xpd=TRUE)
...
if(multipleFiles){
  pandoc.p("\n\n---\n\n")
  pandoc.header("Sample PCA", 3)
  pandoc.p('PCA shown below was performed on the samples only, excluding any control samples. Samples are colored according to run.')
  
  pandoc.header("Pairs plots", 4)
  pandoc.p('Pairs plots show the first 10 PCs plotted against each other. Any clustering by run in the plots indicates potential batch effect(s).')
  ...
  PCAtools::pairsplot(sample_PCA,
                      ...
                      encircle = TRUE, encircleAlpha = 0.15)
}
```
...
if(params$outPlateEffect & multipleFiles){
  pandoc.header("PC correlations with metadata", 4)
  pandoc.p('Sample metadata variables including run and sample matrix, if provided, were correlated against the principal component scores for the first 10 PCs. P-values are adjusted using the Bonferroni correction. Significance scale for adjusted p-values: * < 0.05, ** < 0.01, *** < 0.001.')
...
if(multipleFiles & params$reportType != "webApp"){
  ...
  qs::qsavem(sample_PCA, sample_PCA_eigencor_corvals, sample_PCA_eigencor_pvals, 
             file=file.path(paste0(params$outfolder, "outputFiles"), "batch_effect_test.qs"))
}
```
...
if(multipleFiles & !is.null(params$assayRunInfo)){
  if(do_inst_effect_test){
    ...
    if(nrow(samples_omitted) > 0){
      pandoc.header("Samples omitted from instrument effect test table", 4)
      
      pandoc.p("The following samples were omitted from instrument effect test due to their sample metadata name not matching a NPQ data matrix column name." )
...
if(multipleFiles){
  if(do_inst_effect_test){
    pandoc.header("Instrument effect ANOVA volcano plot", 4)
    
    if(length(inst_effect_results[[1]]$remove_targets) > 0){
      pandoc.p(paste0("NOTE: The following targets were removed from instrument effect test due to zero variance: ", paste(inst_effect_results[[1]]$remove_targets, collapse=', ')))
    }
    ...
    if(n_significant_Ftest_inst == 0){
      pandoc.p(paste0('There were no targets with significant (unadjusted p < ', params$plate_effect_pval_ANOVA, ') ANOVA test instrument effects.'))
    } else if(n_significant_Ftest_inst > 0){
      pandoc.p(paste0(n_significant_Ftest_inst, ' targets showed significant ANOVA test instrument effects (unadjusted p < ', params$plate_effect_pval_ANOVA, ').'))
    }
...
# Save R Data file
# Rename the param variable to avoid conflict when reading multiple qs files
if(params$reportType != "webApp"){
  dir.create(paste0(params$outfolder, "outputFiles"), recursive = T)
  input_params <- params
  qs::qsave(as.list(environment(), all.names = TRUE),
            file=paste0(params$outfolder, "outputFiles/NULISAseq_QC_report.qs")
  )
  rm(params)
}
```
