...
#'   target identifiers, e.g., `"R;CSF-NA"`.
...
calcSampleTargetNAs <- function(curve_quant, sample_matrices){
  ...
  # do not return an NPQ value for reverse curve targets unless sample_matrix is "PLASMA" or "SERUM"
  for (i in seq_along(curve_quant)) {
    ...
    for (j in seq_along(sample_matrices)) {
      ...
      if (target %in% parts || ("R" %in% parts && !sample_matrices[j] %in% c("PLASMA", "SERUM"))) {
...
loadNULISAseq <- function(file,
  ...
  if (AbsAssay){
    ...
    } else{ # Use AQ values if they were embedded
      if("attributes" %in% names(raw)){
        if("AQ" %in% names(raw$attributes)){
          raw$AQ$Data_AQ <- replace(raw$attributes$AQ, is.nan(raw$attributes$AQ), NA)
        }
        if("aM" %in% names(raw$attributes)){
          raw$AQ$Data_AQ_aM <- replace(raw$attributes$aM, is.nan(raw$attributes$aM), NA)
        }
      }
    }
  }
    ...
    if("qcXML" %in% names(raw) && !is.null(raw$qcXML$target)){
      ...
      if(!is.null(raw$qcXML$target$LLOQ) &&
        ...
        raw$AQ$targetAQ_param <- raw$AQ$targetAQ_param[,
          !colnames(raw$AQ$targetAQ_param) %in% c("LLOQ", "LLOQ_aM", "LLOQ_pg_ml")]
        ...
        raw$AQ$targetAQ_param <- merge(raw$AQ$targetAQ_param,
                                       raw$qcXML$target$LLOQ[, lloq_cols],
                                       by="targetName", all.x=TRUE)
      ...
      if(!is.null(raw$qcXML$target$ULOQ) &&
        ...
        raw$AQ$targetAQ_param <- raw$AQ$targetAQ_param[,
          !colnames(raw$AQ$targetAQ_param) %in% c("ULOQ", "ULOQ_aM", "ULOQ_pg_ml")]

        uloq_cols <- intersect(c("targetName", "ULOQ_aM", "ULOQ_pg_ml"),
                               colnames(raw$qcXML$target$ULOQ))
        raw$AQ$targetAQ_param <- merge(raw$AQ$targetAQ_param,
                                       raw$qcXML$target$ULOQ[, uloq_cols],
                                       by="targetName", all.x=TRUE)
      ...
      if(!is.null(raw$qcXML$target$lod) &&
        ...
        raw$AQ$targetAQ_param <- merge(raw$AQ$targetAQ_param,
                                       raw$qcXML$target$lod[, lod_cols],
                                       by="targetName", all.x=TRUE,
                                       suffixes=c("", "_qcXML"))
      }

      # Calculate withinDR if LLOQ/ULOQ are now available
      if("LLOQ" %in% colnames(raw$AQ$targetAQ_param) && "ULOQ" %in% colnames(raw$AQ$targetAQ_param)){
        raw$AQ$withinDR <- ((raw$AQ$Data_AQ_aM >= raw$AQ$targetAQ_param$LLOQ) &
                            (raw$AQ$Data_AQ_aM <= raw$AQ$targetAQ_param$ULOQ))
      }
    }
  }
