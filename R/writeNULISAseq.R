...
#' @param ICs Vector of string(s) or a named list of character string vectors with 
#' internal control target names. Default is NULL which will use the 
#' target type metadata embedded in the xml file to determine the IC. 
#' If non-null, first IC in vector will be used in 
#' intra-plate IC-normalization (usually mCherry). 
#' ICs will be omitted from output file by default unless include_unnorm_counts 
#' and include_IC_counts are both set to TRUE. Can be a single value or vector 
...
#' Default is `FALSE`. This only works when 
...
writeNULISAseq <- function (xml_files, 
  ...
  if (nrow(run_targets) != length(unique(run_targets$targetName))) {
    stop("Inconsistent target metadata was found across runs. Please check targetName, AlamarTargetID, ProteinName, and MW in the Barcode A information.")
  }
  ...
  if (!is.null(sample_info_file)) {
    ...
    } else { # store sample info column names for later to include in final output
  ...
  ############## Do intensity normalization if specified ##############
  ...
  if (include_unnorm_counts==FALSE && include_IC_counts==TRUE){
    warning("include_unnorm_counts must also be set to TRUE for include_IC_counts ",
            "to work. If you want IC counts in the output, please set both ",
  ...
  if (include_unnorm_counts) {
    ...
    if (include_IC_counts==TRUE){
      ...
      sample_metadata <- SampleInfo %>%
        dplyr::select(-Target, -LOD, -UnnormalizedCount, -NPQ,
                      -dplyr::ends_with("_intensity_norm")) %>%
        dplyr::distinct()
      
      # Convert IC counts from wide to long format
      IC_long <- IC_raw_counts %>%
        as.data.frame() %>%
        tibble::rownames_to_column("Target") %>%
        tidyr::pivot_longer(
          cols = -Target, 
          names_to = "SampleName", 
          values_to = "UnnormalizedCount"
        ) %>%
        # Join with sample metadata
        dplyr::left_join(sample_metadata, by = "SampleName") %>%
        # Filter to keep only the samples that are in SampleInfo
        dplyr::filter(!is.na(PlateID)) %>%
        # Add missing columns
        dplyr::mutate(
          LOD = NA_real_,
          NPQ = NA_real_
        ) %>%
        # Reorder to match SampleInfo
        dplyr::select(dplyr::all_of(colnames(SampleInfo)))
      
      # Bind IC data to SampleInfo
      SampleInfo <- dplyr::bind_rows(SampleInfo, IC_long)
      
      if (verbose == TRUE) cat("IC counts added to output.\n")
    }
  } else {
  ...
  if (output_TAP_AQ == TRUE & !anyMissingAQ) {
    ...
    all_targets_AQ <- all_targets %>% 
      ...
      dplyr:: arrange(tolower(TARGETNAME))
    ...
    allData_AQ <- SampleInfo_AQ %>% 
      ...
      # Sort properly: by Target, PlateID, SampleType, SampleName
      dplyr::arrange(tolower(Target), PlateID, SampleType, SampleName) %>%
