...
IPC_wells <- NC_wells <- SC_wells <- vector('list', length(params$xmlFiles))
for (i in 1:length(params$xmlFiles)){
...
```{r, out.width='100%', fig.asp=0.8, results='asis'}
...
PCAplot <- function(data, plateID, scale=TRUE, center=TRUE, title=NULL){
  ...
  if(!is.null(params$SC)){
    ...
    legendText <-  c(levels(plateID), 'Sample', 'IPCs', 'SCs')
  }else{
