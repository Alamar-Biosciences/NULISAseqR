...
#detect_samples <- colnames(plateNorms[[1]]$mCherry$normData)
#detect_samples <- detect_samples[-grep('IPC', detect_samples)]
#detect_samples <- detect_samples[-grep('NC', detect_samples)]
#detect_samples <- detect_samples[-grep('Donor', detect_samples)]
#plate1_detect <- detectability(aboveLOD_matrix=run_lod[[1]]$aboveLOD,
#                               sample_subset=detect_samples,
#                               exclude_targets=ICs)
#detectability_summary <- cbind(format(round(mean(plate1_detect$detectability, na.rm=TRUE), 1), nsmall=1),
#                               format(round(sd(plate1_detect$detectability, na.rm=TRUE), 1), nsmall=1),
#                               median(plate1_detect$detectability, na.rm=TRUE),
#                               min(plate1_detect$detectability, na.rm=TRUE),
#                               max(plate1_detect$detectability, na.rm=TRUE),
#                               paste0(sum(plate1_detect$detectability >= 50),
#                                      ' (',
#                                      format(round(sum(plate1_detect$detectability > 50)/length(plate1_detect$detectability) * 100, 1), nsmall=1),
#                                      '%)'))
#colnames(detectability_summary) <- c('mean',
#                                     'sd',
#                                     'median',
#                                     'min',
#                                     'max',
#                                     'detectable targets n (%)')
#rownames(detectability_summary) <- RunNames
```
...
* **Goal:** NULISAseq 200-plex inflammation panel.
* Sample description:
...
for(i in 1:length(xmlFiles)){
  ...
  cat(knitr::kable(val, caption=paste0("Run ", as.character(i), ": Plate/Sample Layout"), align="c",escape=F) %>% kable_styling(font_size=9))
}
```
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$IC_table, caption=paste0("Run ", as.character(i), ": IC summary"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())  
}
```
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$IPC_table, caption=paste0("Run ", as.character(i), ": IPC summary"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())
}
```
---
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  cat(knitr::kable(runSummarys[[i]]$NC_table, caption=paste0("Run ", as.character(i), ": NC summaray"), align='r', format.args=list(big.mark = ",")) %>% kable_styling())
}
```
...
for(i in 1:length(xmlFiles)){
  ...
  for (j in 1:length(well_types)){
    ...
    inds <- which(grepl(paste0(well_types[[j]], collapse="|"), colnames(data_mCherry_scaled[[i]])))
    if(length(inds) > 0){
...
for(i in 1:length(xmlFiles)){
  ...
  for (j in 1:length(well_types)){
    ...
    inds <- which(grepl(paste0(well_types[[j]], collapse="|"), colnames(corrvals$corr)))
    if(length(inds) > 0){
      mycolors[inds] <- boxplot_colors[j+1]
    }
  }
