...
for( i in 1:length(runs)){
  ...
  par(mfrow=c(ceiling(length(cats)/2), 2))
  
  for(j in 1:length(cats)){
    inds <- which(qcSample$flagName == cats[j])
    qcSample[inds,]$status[23]<-"Fail"
    color <- rep("darkgreen", length(inds))
    xlab <- ""
    xval <- NULL
    if(cats[j] == "TARGETS_ABOVE_LOD"){
      xlab <- "Detectability %"
    ...
    dotchart(x=as.numeric(xval), xlab=xlab, 
             ...
             main=paste0("Plate ", as.character(i), ": ", cats[j]), 
             cex=0.45, color=color )
    legend('topleft', legend=c('Pass', "Fail"), col=c('darkgreen', 'red'), lty=1, cex=0.4, bty='n')
  } 
}
```
