...
#  )
  '20230109_TAP_covid_plate01_nononmatch_XML.xml',
  '20230109_TAP_covid_plate02_nononmatch_XML.xml')
xmlFilePlateNames <- rep("", times=length(xmlFiles)) #Plate prefix
...
for(i in 1:length(xmlFiles)){
  ...
  if (xmlFilePlateNames[i] != ""){
    PlateNames[i] <- paste0(PlateNames[i], "(", xmlPlateNames[i], ")")
  }
}
...
```{r, results='asis'}
for(i in 1:length(xmlFiles)){
  val <- matrixify(runs[[i]]$samples, rowAnnotName, colAnnotName, plate=T)
...
for(i in 1:length(xmlFiles)){
  val <- matrixify(runs[[i]]$samples, rowAnnotName, colAnnotName)
  ...
  val <- vals
  val[which(!is.na(vals))] <- logVals
  plateHeatmap(as.numeric(val), title=paste0('Plate ', as.character(i), ": ", 'log2(total counts)'), cex=0.5, digits=digitsTotal,
               relative=heatMapRel, cex.axis=0.5)
  for (j in 1:length(ICs)){
    val <- vals
    val[ which(!is.na(vals))] <- runs[[i]]$Data[ICs[j],]
    plateHeatmap(as.numeric(val), title=paste0("Plate ", as.character(i),": ",  ICs[j]), cex=0.5, digits=digitsmCherry, relative=heatMapRel, cex.axis=0.5)
  }
}
```
...
for(i in 1:length(xmlFiles)){
  dataPCAunnormal <- cbind(dataPCAunnormal, removeTargetsSamples(runs[[i]]$Data, NC, IPC, Bridge))
  dataPCAnormed <- cbind(dataPCAnormed, removeTargetsSamples(data_mCherry_scaled[[i]], NC, IPC, Bridge))
  dataPCAnormedIPC <- cbind(dataPCAnormedIPC, removeTargetsSamples(mCherry_IPC$interNormData[[i]], NC, IPC, Bridge))
  dataPCAnormedIN <- cbind(dataPCAnormedIN, removeTargetsSamples(mCherry_IN$interNormData[[i]], NC, IPC, Bridge))
  plateVar <- c(plateVar, rep(paste0("Plate",i),length(removeTargetsSamples(data_mCherry_scaled[[i]], NC, IPC, Bridge)[1,])))
}
