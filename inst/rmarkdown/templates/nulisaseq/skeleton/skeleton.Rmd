...
for(i in 1:length(params$xmlFiles)){
  ...
  cat(knitr::kable(val, caption=paste0("Plate ", as.character(i), ": Plate/Sample Layout"), align="c", escape=FALSE, table.attr=table.attr) %>%
...
intraCV_vals <- t(interleave(t(intraCV_unnorm), t(intraCV_mCherry2), drop=FALSE))
...
boxplot(intraCV_vals
        ...
        main='Intra-plate CV (* - Normalized)', horizontal=TRUE, cex.axis=0.5, lex.order=TRUE,
...
boxplot(interCV_results, las=1, xlab='inter-plate CV (%)',
        ylim=c(0, max(interCV_results, na.rm=TRUE)), col=alamarColorPalette(5), horizontal=TRUE)
